...
struct TestParam {
  ...
  std::string ToString() const {
    return absl::StrCat(
        refcount_is_one ? "Private" : "Shared",
        with_capacity ? "" : "_NoCapacity",
        (input_share_mode == kPrivate) ? "" : (input_share_mode == kShared)
                                                  ? "_SharedInput"
...
TEST_P(CordRingSubTest, SubRingFromLargeExternal) {
  ...
  const char* flats[] = {
      "abcdefghijklmnopqrstuvwxyz", large_string.c_str(),
...
TEST_F(CordRingTest, Find) {
  constexpr const char* flats[] = {"abcdefghij", "klmnopqrst",   "uvwxyz",
