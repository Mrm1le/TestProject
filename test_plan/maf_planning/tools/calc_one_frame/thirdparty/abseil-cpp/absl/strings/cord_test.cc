...
TEST_P(CordTest, TryFlatCommonlyAssumedInvariants) {
  ...
  absl::string_view fragments[] = {"A fragmented test", " cord",
                                   " to test subcords", " of ", "a",
                                   " cord for",
                                   " each chunk "
                                   "returned by the ",
                                   "iterator"};
...
TEST_P(CordTest, CompareRandomComparisons) {
  ...
  absl::Cord a[] = {MakeExternalCord(n), absl::Cord("ant"),
                    absl::Cord("elephant"), absl::Cord("giraffe"),
                    absl::Cord(std::string(GetUniformRandomUpTo(&rng, 100),
                                           GetUniformRandomUpTo(&rng, 100))),
                    absl::Cord(""), absl::Cord("x"), absl::Cord("A"),
                    absl::Cord("B"), absl::Cord("C")};
...
TEST_P(CordTest, ExternalMemoryRemovePrefixSuffix) {
  ...
  for (int offset = 0; offset <= s.size(); offset++) {
    for (int length = 0; length <= s.size() - offset; length++) {
      ...
      EXPECT_EQ(s.substr(offset, length), std::string(result)) << offset << " "
                                                               << length;
    }
  }
}
...
TEST_P(CordTest, SmallBufferAssignFromOwnData) {
  ...
  for (size_t pos = 0; pos < contents.size(); ++pos) {
    for (size_t count = contents.size() - pos; count > 0; --count) {
      ...
      EXPECT_EQ(c, contents.substr(pos, count)) << "pos = " << pos
                                                << "; count = " << count;
    }
  }
}
