...
void CheckUInt64(uint64_t x) {
  ...
  EXPECT_EQ(expected, std::string(&buffer[1], generic_actual)) << " Input "
                                                               << x;
...
void VerifySimpleAtoiGood(in_val_type in_value, int_type exp_value) {
  ...
  EXPECT_TRUE(SimpleAtoi(s, &x)) << "in_value=" << in_value << " s=" << s
                                 << " x=" << x;
...
TEST(NumbersTest, Atoi) {
  ...
  VerifySimpleAtoiGood<absl::int128>(std::numeric_limits<uint64_t>::max(),
                                     std::numeric_limits<uint64_t>::max());
  VerifySimpleAtoiGood<absl::int128>(std::numeric_limits<absl::int128>::min(),
                                     std::numeric_limits<absl::int128>::min());
  VerifySimpleAtoiGood<absl::int128>(std::numeric_limits<absl::int128>::max(),
...
void VerifySimpleHexAtoiGood(in_val_type in_value, int_type exp_value) {
  ...
  int_type x = static_cast<int_type>(~exp_value);
  EXPECT_TRUE(SimpleHexAtoi(s, &x)) << "in_value=" << std::hex << in_value
                                    << " s=" << s << " x=" << x;
  EXPECT_EQ(exp_value, x);
  x = static_cast<int_type>(~exp_value);
...
TEST_F(SimpleDtoaTest, ExhaustiveDoubleToSixDigits) {
  ...
  if (mismatches.empty()) {
    ...
    for (size_t i = 0; i < mismatches.size(); ++i) {
      ...
      ABSL_RAW_LOG(
          ERROR, "%s",
          absl::StrCat("Mismatch #", i, "  d=", d, " (", ToNineDigits(d), ")",
                       ...
                       (SixDigitsToBuffer(d, b2), b2), " vs snprintf=",
                       (snprintf(b1, sizeof(b1), "%g", d), b1), " After.=.",
                       PerfectDtoa(after), " ",
