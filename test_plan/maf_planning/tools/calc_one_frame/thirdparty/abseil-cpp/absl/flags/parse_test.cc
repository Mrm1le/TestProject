...
TEST_F(ParseTest, TestValidBoolArg) {
  ...
  const char* in_args3[] = {
      "testbin", "--bool_flag=true",
  };
  TestParse(in_args3, 1, 1.1, "a", true);

  const char* in_args4[] = {
      "testbin", "-bool_flag=false",
  };
...
TEST_F(ParseTest, TestFlagfileInFlagfile) {
  ...
  constexpr const char* const ff3_data[] = {
      "--flagfile=$0/parse_test.ff1", "--flagfile=$0/parse_test.ff2",
  };
...
TEST_F(ParseDeathTest, TestInvalidFlagfiles) {
  ...
  constexpr const char* const ff4_data[] = {"--unknown_flag=10"};

  const char* in_args1[] = {
      "testbin",
      GetFlagfileFlag({{"parse_test.ff4", absl::MakeConstSpan(ff4_data)}},
                      flagfile_flag),
  };
  EXPECT_DEATH_IF_SUPPORTED(InvokeParse(in_args1),
  ...
  const char* in_args2[] = {
      "testbin",
      GetFlagfileFlag({{"parse_test.ff5", absl::MakeConstSpan(ff5_data)}},
                      flagfile_flag),
  };
  EXPECT_DEATH_IF_SUPPORTED(InvokeParse(in_args2),
...
TEST_F(ParseTest, TestReadingFlagsFromEnvMoxedWithRegularFlags) {
  const char* in_args1[] = {
      "testbin", "--bool_flag=T", "--tryfromenv=int_flag,bool_flag",
      "--int_flag=-21",
  };
...
TEST_F(ParseTest, TestIgnoreUndefinedFlags) {
  const char* in_args1[] = {
      "testbin", "arg1", "--undef_flag=aa", "--int_flag=21",
  };
  ...
  const char* in_args2[] = {
      "testbin", "arg1", "--undef_flag=aa", "--string_flag=AA",
  };
