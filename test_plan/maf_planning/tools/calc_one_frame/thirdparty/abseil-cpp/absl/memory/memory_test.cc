...
TEST(AllocatorTraits, RebindWithPrivateInheritance) {
  ...
  EXPECT_TRUE((std::is_same<AllocWithPrivateInheritance<int>,
                            absl::allocator_traits<AllocWithPrivateInheritance<
                                char>>::rebind_alloc<int>>::value));
}
...
struct FullMockAllocator {
  ...
  MOCK_METHOD(size_t, max_size, (), (const));
