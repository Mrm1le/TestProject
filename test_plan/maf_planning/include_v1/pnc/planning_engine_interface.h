#ifndef CP_MSQUARE_DECISION_PLANNING_COMMON_PLANNING_ENGINE_INTERFACE_H_
#define CP_MSQUARE_DECISION_PLANNING_COMMON_PLANNING_ENGINE_INTERFACE_H_

#include <memory>

#include "cp_pnc.h"
#include "maf/base.hpp"
#include "msd/worldmodel/cp_worldmodel_generator.h"
...
using NPPExtraDataListener = std::function<void(const maf_std::Header &planning_extra_data)>;
using NPPProtoDebugListener = std::function<void(const maf_std::Header &planning_proto_debug)>;
...
public:
  ...
  MSD_API virtual void feed_perception_vision_landmark(
      const std::shared_ptr<maf_perception_interface::RoadLinePerception>
          perception_vision_landmark) = 0;
  ...
  MSD_API virtual void
  set_extra_data_callback(NPPExtraDataListener callback) = 0;
  MSD_API virtual void
  set_proto_debug_callback(NPPProtoDebugListener callback) = 0;
};
