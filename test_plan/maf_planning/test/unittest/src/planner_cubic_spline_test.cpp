...
TEST(PATH_PLANNER_TEST, CubicSpline)
{
	...
	std::array<double, N> s_at_control_points = {0, 20, 40, 60, 80, 100, 120};
	std::array<double, N> x_at_control_points = {-4.0, 1.6, 0.0, -1.0, -2.0, 0., 0.};
	...
	EXPECT_EQ(all_quad_pt_s.size(), 30);
	EXPECT_NEAR(all_quad_pt_s[10], 40.93820, 1e-4);
	EXPECT_NEAR(all_quad_pt_x[10], -0.07867, 1e-4);
	EXPECT_NEAR(all_quad_pt_dx[10], -0.07903, 1e-4);
	EXPECT_NEAR(all_quad_pt_d2x[10], 0.00996, 1e-4);
	EXPECT_NEAR(all_quad_pt_d3x[10], -0.00106, 1e-4);
	std::cout << "first test passed. " << std::endl;

    // Test at sample point 2
	EXPECT_NEAR(all_quad_pt_s[15], 60.93820, 1e-4);
	EXPECT_NEAR(all_quad_pt_x[15], -1.08104, 1e-4);
	EXPECT_NEAR(all_quad_pt_dx[15], -0.09059, 1e-4);
	EXPECT_NEAR(all_quad_pt_d2x[15], -0.00836, 1e-4);
	EXPECT_NEAR(all_quad_pt_d3x[15], 0.00202, 1e-4);
