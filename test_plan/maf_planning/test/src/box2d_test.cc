...
TEST(Box2dTest, HasOverlap) {
  ...
  EXPECT_TRUE(box1.HasOverlap(LineSegment2d({4, -4}, {-4, 4})));
  EXPECT_TRUE(box5.HasOverlap(LineSegment2d({0, 0}, {-4, -6})));
...
TEST(Box2dTest, IsPointIn) {
  ...
  EXPECT_TRUE(box5.IsPointIn({-2, -3}));
...
TEST(Box2dTest, TestLateralDistanceToLine)
{
  Box2d box(Vec2d(0, 0), 0, 2, 1);
  // cross over
  LineSegment2d line1(Vec2d(1, 1), Vec2d(-1, -1));
  EXPECT_NEAR(box.LateralDistanceTo(line1), -box.width()/2, 1e-5);
  // both positive
  line1 = LineSegment2d(Vec2d(1, 0.5), Vec2d(-1, 1));
  EXPECT_NEAR(box.LateralDistanceTo(line1), 0.5-box.width()/2, 1e-5);
  // both negative
  line1 = LineSegment2d(Vec2d(1, -0.5), Vec2d(-1, -1));
  EXPECT_NEAR(box.LateralDistanceTo(line1), 0.5-box.width()/2, 1e-5);
  
  // line start beyong box range
  line1 = LineSegment2d(Vec2d(2, 0.5), Vec2d(-1, 1));
  EXPECT_NEAR(box.LateralDistanceTo(line1), 0.5-box.width()/2, 1e-5);
  // line end beyong box range
  line1 = LineSegment2d(Vec2d(1, -0.5), Vec2d(-2, -1));
  EXPECT_NEAR(box.LateralDistanceTo(line1), 0.5-box.width()/2, 1e-5);

  line1 = LineSegment2d(Vec2d(1, 0), Vec2d(-1, 0));
  box.RotateFromCenter(M_PI_4);
  box.Shift(Vec2d(0, 10));
  EXPECT_DOUBLE_EQ(std::sqrt(2)/2.0*9-box.width()/2, box.LateralDistanceTo(line1));

  line1 = LineSegment2d(Vec2d(1, 0), Vec2d(-1, 0));
  box = Box2d(Vec2d(0, 0), 0, 2, 1);
  box.RotateFromCenter(M_PI_4);
  box.Shift(Vec2d(0, 3.5));
  EXPECT_DOUBLE_EQ(std::sqrt(2)/2.0*2.5-box.width()/2, box.LateralDistanceTo(line1));
}

TEST(Box2dTest, TestLateralDistanceToBox)
{
  Box2d ego_box(Vec2d(0,0), 0, 2, 1);
  Vec2d obj_center(2,2);
  std::vector<Box2d> objs(4);
  double headings[4] = {0, M_PI_4, M_PI_4, 0};
  double lengths[4] = {1, 2, 1, 2};
  double widths[4] = {1, 1, 1, 1};
  double expects[4];
  for(int i = 0; i < 4; ++i)
  {
    Box2d obj_box(obj_center, headings[i], lengths[i], widths[i]);
    LineSegment2d nearest_edge(obj_box.GetAllCorners().front(), obj_box.GetAllCorners().back());
    objs[i] = obj_box;
    expects[i] = ego_box.LateralDistanceTo(nearest_edge);
  }

  for(int i = 0; i < 4; ++i)
  {
    EXPECT_DOUBLE_EQ(expects[i], ego_box.LateralDistanceTo(objs[i]));
  }
}
