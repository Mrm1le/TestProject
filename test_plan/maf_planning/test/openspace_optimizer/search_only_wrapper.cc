...
int construct_planner(const char *config_file_name,
                      const char *arg_stra_odca_file,
                      const char *car_param_file, TrajectoryPoint target_state,
                      Box2d map_boundary) {
  ...
  Pose2D target_pose(target_state.path_point_.x_, target_state.path_point_.y_,
                     target_state.path_point_.theta_);
  if (msquare::HybridAstarConfig::GetInstance()->planning_core == 0) {
    planner =
        std::make_shared<msquare::HybridAstar>(target_pose, 0, g_map_boundary);
  } else {
...
double plan(const char *arg_stra_odca_file) {
  ...
  if (msquare::parking::genOpenspacePath(planner, input, result)) {
...
int get_result_size() { return result.x.size(); }

int get_search_size() { return search_points_pair.size(); }

void get_result(TrajectoryPoint *trajectory, int *num_segments,
                int *iteration_times, char *debug_string) {
  for (size_t i = 0; i < result.x.size(); ++i) {
    ...
    trajectory[i].wheel_base_offset_ =
        result.wheel_base_offset.at(i); // unused by then
  }
