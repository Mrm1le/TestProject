...
public:
  ...
  EntryItem();
  explicit EntryItem(const int);
  explicit EntryItem(const double);
  explicit EntryItem(const bool);
  explicit EntryItem(const std::string &);
  explicit EntryItem(const char *);
  explicit EntryItem(const std::vector<int> &);
  explicit EntryItem(const std::vector<double> &);
  explicit EntryItem(
      const std::vector<bool> &); // parasoft-suppress AUTOSAR-A18_1_2-a
  explicit EntryItem(const std::vector<std::string> &);
...
static const std::vector<bool>
    EMPTY_ENTRY_ITEM_BOOL_ARRAY; // parasoft-suppress AUTOSAR-A18_1_2-a
...
public:
  ...
  virtual const int &digit_int_value() const {
    assert(ERROR_NUMBER_VALUE);
    abort();
  }
  ...
  virtual bool is_int_array() const {
    assert(ERROR_NUMBER_VALUE);
    abort();
  }
  virtual bool is_double_array() const {
    assert(ERROR_NUMBER_VALUE);
    abort();
  }
  virtual bool is_bool_array() const {
    assert(ERROR_NUMBER_VALUE);
    abort();
  }
  virtual bool is_string_array() const {
    assert(ERROR_NUMBER_VALUE);
    abort();
  }
...
public:
  ...
  ArgumentParser &
  default_value(const std::vector<bool>
                    &default_value); // parasoft-suppress AUTOSAR-A18_1_2-a
  ...
  ArgumentParser &
  const_value(const std::vector<bool>
                  const_value); // parasoft-suppress AUTOSAR-A18_1_2-a
  // help
  ...
  ArgumentParser &
  choices(const std::vector<bool>
              &bool_array); // parasoft-suppress AUTOSAR-A18_1_2-a
  ...
  // parse arguments
  ArgValue parse_args_any_type(int argc, const char *argv[]);
  std::map<std::string, std::string> parse_args_string(int argc,
                                                       const char *argv[]);
...
public:
  explicit DigitsIntValue(const int value) : value_(value) {}
  ...
  void dump(std::string &output) const override {
    ...
    (void)snprintf(temp, INT_TO_STRING_SIZE, "%d", value_);
    output += temp;
  }
...
public:
  explicit DigitsDoubleValue(const double value) : value_(value) {}
  const double &digit_double_value() const override { return value_; }
  ValueType value_type() const override { return value_type_; }
  void dump(std::string &output) const override {
    char temp[DOUBLE_TO_STRING_SIZE];
    (void)snprintf(temp, DOUBLE_TO_STRING_SIZE, "%.3f", value_);
    output += temp;
  }
...
public:
  BoolArrayValue(
      const std::vector<bool> &value) // parasoft-suppress AUTOSAR-A18_1_2-a
      : value_(value.begin(), value.end()) {}
  ...
  void dump(std::string &output) const override {
    output += "{";
    bool begin_flag = true;
    for (auto item : value_) { // parasoft-suppress AUTOSAR-A18_1_2-a
                               // "third-party code interface"
      if (!begin_flag) {
        output += ",";
      }
      EntryItem(item).dump(output);
...
private:
  std::vector<bool> value_; // parasoft-suppress AUTOSAR-A18_1_2-a "third-party
                            // code interface"
...
inline ArgumentParser &ArgumentParser::default_value(
...
inline ArgumentParser &ArgumentParser::const_value(
    const std::vector<bool> // parasoft-suppress AUTOSAR-A18_1_2-a
        const_value) {      // parasoft-suppress AUTOSAR-A18_1_2-a
  check_error(now_entry_.value_type == ValueType::NONE ||
                  now_entry_.value_type == ValueType::BOOLEAN,
...
inline ArgumentParser &
ArgumentParser::choices(const std::vector<bool> &
                            bool_array) { // parasoft-suppress AUTOSAR-A18_1_2-a
  check_error(now_entry_.value_type == ValueType::NONE ||
                  now_entry_.value_type == ValueType::BOOLEAN,
              "not boolean type");
...
inline void ArgumentParser::done() {
  ...
  now_entry_ =      // parasoft suppress AUTOSAR-A8_5_0-a-2 "3-party code"
      ParamEntry(); // parasoft suppress AUTOSAR-A8_5_0-a-2 "3-party code"
  init();
}
...
inline bool ArgumentParser::is_entry_item_equal(const EntryItem item1,
  ...
  if (item1.is_double_array()) {
    ...
    for (size_t i = 0; i < item2_value.size(); i++) {
      if (std::fabs(item1_value[i] - item2_value[i]) > 1e-8) {
        return false;
      }
    }
    return true;
  ...
  if (item1.is_bool_array()) {
    auto item1_value =            // parasoft-suppress AUTOSAR-A18_1_2-a
        item1.bool_array_value(); // parasoft-suppress AUTOSAR-A18_1_2-a
    auto item2_value =            // parasoft-suppress AUTOSAR-A18_1_2-a
        item2.bool_array_value(); // parasoft-suppress AUTOSAR-A18_1_2-a
    if (item1_value.size() !=     // parasoft-suppress AUTOSAR-A18_1_2-a
        item2_value.size()) {     // parasoft-suppress AUTOSAR-A18_1_2-a
      return false;
    }
    for (size_t i = 0; i < item2_value.size(); i++) {
      if (item1_value[i] != item2_value[i]) {
  ...
  if (item1.is_double()) {
    if (std::fabs(item1.digit_double_value() - item2.digit_double_value()) >
        1e-8) {
      return false;
    }
  }
...
inline bool ArgumentParser::parse_choices(const EntryItem item_value,
  ...
  if (temp.is_double_array()) {
    ...
    for (auto item : temp_array) {
      if (std::fabs(item - item_value.digit_double_value()) < 1e-8) {
        return true;
      }
    }
  }
  ...
  if (temp.is_bool_array()) {
    auto temp_array =              // parasoft-suppress AUTOSAR-A18_1_2-a
        temp.bool_array_value();   // parasoft-suppress AUTOSAR-A18_1_2-a
    for (auto item : temp_array) { // parasoft-suppress AUTOSAR-A18_1_2-a
...
inline void ArgumentParser::parse_nargs(ParamEntry &entry, int &p_input_index,
  ...
  if (entry.nargs == NargsType::NARGS_QUESTION) { // nargs = '?'
    ...
    (void)parse_choices(item_result, entry);
    p_input_index++;
    ...
    while (true) {
      ...
      (void)parse_choices(item_value, entry);
    ...
    if (item_result.empty()) {
      ...
      if (item_result[0].is_int()) {
        ...
        std::vector<bool> temp_result; // parasoft-suppress AUTOSAR-A18_1_2-a
                                       // "third-party code interface"
        for (auto item : item_result) {
...
// parse arguments
inline ArgValue ArgumentParser::parse_args_any_type(int argc,
                                                    const char *argv[]) {
...
ArgumentParser::parse_args_string(int argc, const char *argv[]) {
    ...
    while (i < entries.size() && entries[i].required) {
      ...
      if (!entries[i].default_value.string_value().empty()) {
        (void)printf(" DEFAULT %s",
                     entries[i].default_value.string_value().c_str());
      }
      (void)printf("\n");
      i++;
    }
  ...
  if (i < entries.size()) {
    ...
    while (i < entries.size()) {
      ...
      if (!entries[i].short_key.empty()) {
        exist_prefix = true;
        (void)printf("-%s", entries[i].short_key.c_str());
      }
      if (!entries[i].long_key.empty()) {
        if (exist_prefix) {
          (void)printf(", --%s", entries[i].long_key.c_str());
        } else {
          (void)printf("--%s", entries[i].long_key.c_str());
        }
      }
      ...
      if (entries[i].choices_array.value_type() == ValueType::INT_ARRAY ||
          ...
          entries[i].choices_array.value_type() == ValueType::STRING_ARRAY ||
          entries[i].choices_array.value_type() == ValueType::BOOL_ARRAY) {
        (void)printf(" CHOOSE FROM %s",
                     entries[i].choices_array.dump().c_str());
      }
      if (!entries[i].default_value.string_value().empty()) {
        (void)printf(" DEFAULT %s",
                     entries[i].default_value.string_value().c_str());
