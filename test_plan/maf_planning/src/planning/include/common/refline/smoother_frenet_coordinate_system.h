...
// particle is considered as stopped  const double m_fcs_params.precision =
// 0.01; const double m_fcs_params.step_s = 0.3; const double
// m_fcs_params.coarse_step_s = 1.0; const double
// m_fcs_params.optimization_gamma = 0.5;  // Gradient descending parameter
// const int m_fcs_params.max_iter = 15;  // Max iteration for the Newton's
// method
...
private:
  ...
  double m_length;
  // std::unique_ptr
  std::unique_ptr<Spline2d> spline;
...
public:
  ...
  TRANSFORM_STATUS CartCoord2FrenetCoord(const Point2D &cart,
                                         Point2D &frenet) const;
  //! Cartesian coordinate to frenet coordinate
  ...
  */
  ...
  double GetRefCurveHeading(double s) const {
    // mph_assert(s <= m_length && s >= 0);
    if (!(s <= m_length && s >= 0)) {
      MSD_LOG(WARN, "GetRefCurveHeading, invalid s: %f", s);
    }
  ...
  double GetRefCurveAvgHeading(double s, double delta_s) const {
    // mph_assert(s <= m_length && s >= 0);
    if (!(s <= m_length && s >= 0)) {
      MSD_LOG(WARN, "GetRefCurveAvgHeading, invalid s: %f", s);
    }
  ...
  double GetRefCurveCurvature(double s) const {
    // mph_assert(s <= m_length && s >= 0);
    if (!(s <= m_length && s >= 0)) {
      MSD_LOG(WARN, "GetRefCurveCurvature, invalid s: %f", s);
    }
    return m_k_s(s);
  }
  ...
  double GetRefCurveDCurvature(double s) const {
    // mph_assert(s <= m_length && s >= 0);
    if (!(s <= m_length && s >= 0)) {
      MSD_LOG(WARN, "GetRefCurveDCurvature, invalid s: %f", s);
    }
  ...
  Point2D GetRefCurvePoint(double s) const {
    // mph_assert(s <= m_length && s >= 0);
    if (!(s <= m_length && s >= 0)) {
      MSD_LOG(WARN, "GetRefCurvePoint, invalid s: %f", s);
    }
  ...
  vector<double> GetCurveParameters() const { return curvature_fit_parameters; }
  void VerifyCurveFit();
};
