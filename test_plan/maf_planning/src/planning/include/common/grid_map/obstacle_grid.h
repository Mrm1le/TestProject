...
#include "common/grid_map/search_node.hpp"
#include "common/grid_map/tmp_usage_only_global_variables.hpp"
...
public:
  ...
  static constexpr int DEBUG_X = 1053;
  static constexpr int DEBUG_Y = 4;
  static constexpr int DEBUG_POLYGON = 29;
...
public:
  ...
  bool init(int rows, int cols, double res, double origin_x, double origin_y) {
    ...
    grid_data_f32_buffer_.create(rows, cols, CV_32F);
  ...
  bool constructFromSbpObstacleMultiHeights(
    ...
    std::vector<SbpObstaclePtr>
        obs_ptrs_by_heights[(int)ObstacleHeightType::HEIGHT_TYPE_NUM];
    for (std::size_t i = 0; i < obs_ptrs.size(); i++) {
      ObstacleHeightType height = obs_ptrs[i]->getHeightType();
      if (height == ObstacleHeightType::UNKNOWN) {
        height = ObstacleHeightType::HIGH;
      }
      obs_ptrs_by_heights[(int)height].emplace_back(obs_ptrs[i]);
    }

    for (std::size_t i = 0; i < enabling_heights_.size(); i++) {
      ObstacleHeightType height = enabling_heights_[i];
      constructFromSbpObstacle(obs_ptrs_by_heights[(int)height], height);
    }
    return true;
  }
  ...
  bool constructFromSbpObstacleMultiHeights(
      const std::vector<SbpObstaclePtr> &obs_ptrs,
      MultiCircleFootprintModel &footprint, const HAS2SearchNode &pose,
      double inflation_high, double inflation_low) {
  ...
  bool constructFromSbpObstacle(const std::vector<SbpObstaclePtr> &obs_ptrs,
                                ObstacleHeightType height,
                                const MultiCircleFootprintModel &footprint,
                                int footprint_model_index,
                                const HAS2SearchNode &pose, double inflation) {
    const auto &model = footprint.models_[footprint_model_index];
    std::vector<int> circle_used(footprint.circles_local_.size(), 0);
    for (int circle_index : model.circle_index_) {
      circle_used[circle_index] = 1;
    }
    for (int model_index : model.include_model_) {
      for (int circle_index : footprint.models_[model_index].circle_index_) {
        circle_used[circle_index] = 1;
      }
    }

    std::vector<MultiCircleFootprintModel::Circle> circles;
    circles.reserve(footprint.circles_local_.size());
    for (std::size_t i = 0; i < circle_used.size(); i++) {
      if (circle_used[i] == 1) {
        circles.push_back(footprint.circles_local_[i]);
      }
    }
    std::vector<double> needed_r_square(circles.size());
    for (std::size_t i = 0; i < circles.size(); i++) {
      double needed_r = circles[i].radius_ + inflation;
    ...
    for (std::size_t i = 0; i < obs_ptrs.size(); i++) {
      ...
      for (std::size_t j = 0; j < obs_points.size(); j++) {
        ...
        for (std::size_t k = 0; k < circles.size(); k++) {
          double dx = obs_points[j].x() - circles[k].center_.x();
          double dy = obs_points[j].y() - circles[k].center_.y();
        }
        if (valid) {
  ...
  void constructGridFillVoroniConvexPolyWithDistanceFloat(
      cv::Mat &img, cv::Point2f *vf, int npts, float voroni_center_x,
      float voroni_center_y, int debug_only_voroni_id) {
    ...
    //      1. （重要）FillConvexPoly
    //      计算完每行的[x_left,x_right]后，使用memset填充固定的像素值，这里将“填充颜色”替换成逐grid（pixel）计算到最近障碍物的distance。
    ...
    //      2.
    //      （重要）原函数只支持由整数点构成的多边形的计算，实际我们的需求是填充浮点数构成的多边形的“内整点”，这里修改了实现使得支持浮点输入。
    //      3. （重要）FillConvexPoly
    //      除了填充多边形内部外，还有填充多变形的边缘的算法（同时支持多种线型包括AntiAlis），我们不需要，予以删除，在边缘直接用填充算法取代
    //      4. shift功能不需要直接删掉
    //      5. 其他细节修改，不一一说明
    // 以下是填充Polygon的流程解释
    // =====================================================================================
    ...
    //
    // Opencv convex
    // polygon的填充采用的是水平行扫描的方式，即从miny（多边形的最小y）坐标开始，按顺序递增y，逐行扫描直到maxy
    //    对于每一个y行，都计算的到该行与多边形的2个交点的x坐标。然后填充介于[x_left,x_right]之间像素即达成结果。由于任意convex
    //    polygon，任一水平行至多与2条边相交，所以该方法可以成立。
    //    在扫描的过程中，高效实现的关键是维护2个“edge”。即与当前的y行相交的2条边的信息。edge中只有非常有限的信息在必要的时候更新计算，因此速度可以非常快。
    //
    // edge所维护的信息内容
    //
    //     -
    //     idx：edge的“目标顶点“的index。常规理解一条边必须有一个起点和一个终点。但Edge的”起点信息“
    //     始终被维护在（x，y）里，所以只需要明确edge的终点即可
    //
    //     -
    //     x：第y行（当前扫描行），与edge相交的交点x，也可以认为是当前edge的起点位置。
    //     (x,y)就是当前在该edge上扫描所在的起点坐标。由于y已经在外层循环里维护了，这里只需要记录x即可。
    //
    //     -
    ...
    //          在edge耗尽（y>=ye）的时候，其中一条edge朝着idx +1
    //          的方向更新，另一条edge朝着idx-1（也就是 (idx +
    //          npts-1)%npts）的方向更新到下一个目标顶点。
    //
    // 填充时刻：
    //     1. 常规填充 ：
    //     y递增1时，计算左edge和右edge的x，然后填充介于介于[x_left,x_right]的grid
    //     2. 水平边填充： 当出现水平边（v[i].y ==
    //     v[i+1].y）时，会在y>edige[i-1].y，edge由i-1更新到i的时候，跳过i，直接更新到i+1，，因此此时需要额外填充介于
    //     y[v[i].x，v[i+1].x]的像素
    ...
    struct {
      ...
      std::string getInfo() {
        char buf[512];
        sprintf(buf, "idx = %d di = %d x = %f dx = %f ye = %f(%d)", idx, di, x,
                dx, ye, ye_floor);
        return buf;
      }
    } edge[2];
    ...
    for (y = ymin_ceil; y <= ymax_floor; y++) {
      ...
      for (i = 0; i < 2; i++) {
        if (y > edge[i].ye_floor ||
            y == ymin_ceil) { // ready to find next available edge
                ...
                for (x = xx1; x <= xx2; x++) {
                  ...
                  if (dis < ptr[x]) {
                    ...
                    if (TmpGlobals::CONFIG_OBSTACLE_GRID_DEBUG) {
                      voroni_buffer_.at<int>(y, x) = debug_only_voroni_id;
                    }
                  }
                }
              }
  ...
  bool __test_constructFromObstaclePoints(
  ...
  bool constructFromSbpObstacle(const std::vector<SbpObstaclePtr> &obs_ptrs,
                                ObstacleHeightType height) {
    std::vector<cv::Point2f> obstacle_points_in_grid;
    convertSbpObstacleToPointsInGrid(obs_ptrs, obstacle_points_in_grid);
  ...
  bool constructFromPointsInGrid(const std::vector<cv::Point2f> &points_in_grid,
    ...
    if (points_in_grid.size() == 0) {
      ...
      grid_data_u16_[(int)height].resize(
          rows_ * cols_, std::numeric_limits<std::uint16_t>::max());
      return true;
    }
    cv::Subdiv2D subdiv(cv::Rect(0, 0, cols_, rows_));
    for (std::size_t i = 0; i < points_in_grid.size(); i++) {
      if (points_in_grid[i].x >= 0 && points_in_grid[i].y >= 0 &&
          points_in_grid[i].x <= cols_ - 1 &&
          points_in_grid[i].y <= rows_ - 1) {
    ...
    // auto fill_polygon_timer =
    // msd_planning::utils::TotalTimer<true>("fill_convex_poly");

    // get_voroni_timer.Tic();
    subdiv.getVoronoiFacetList(std::vector<int>(), voroni_faces_buffer_,
                               voroni_centers_buffer_);
    ...
    for (size_t i = 0; i < voroni_faces_buffer_.size(); i++) {
      ...
      // constructGridFillVoroniConvexPolyWithDistance2(grid_data, (
      // cv::Point2l*)facei_converted_to_point2i.data(),
      // facei_converted_to_point2i.size(),
      //   voroni_centers_buffer_[i].x, voroni_centers_buffer_[i].y, (int)i);
      // fill_polygon_timer.Tic();
      constructGridFillVoroniConvexPolyWithDistanceFloat(
          grid_data_f32_buffer_, (cv::Point2f *)voroni_faces_buffer_[i].data(),
          voroni_faces_buffer_[i].size(), voroni_centers_buffer_[i].x,
          voroni_centers_buffer_[i].y, (int)i);
  ...
  double getDistance(const MultiCircleFootprintModel::Circle &circle,
                     ObstacleHeightType height) {
    int gx = std::round((circle.center_.x() - origin_x_) * inv_res_);
  ...
  MultiModelMinDistances calcMultiModelMinDistance(
    ...
    for (std::size_t hi = 0; hi < enabling_heights_.size(); hi++) {
      ...
      for (std::size_t mi = 0;
           mi < footprint_model->models_by_heights_[(int)height].size(); mi++) {
        const MultiCircleFootprintModel::Model &model =
            footprint_model->models_by_heights_[(int)height][mi];

        double min_distance;
        if (model.include_model_.empty()) {
          min_distance = std::numeric_limits<double>::infinity();
        } else {
          min_distance =
              result.distances[model.include_model_[0]]; // max one include
        }
        for (int ci : model.circle_index_) {
          double dis = getDistance(footprint_model->circles_local_[ci], height);
          if (dis < min_distance)
            min_distance = dis;
        }
        result.distances[model.model_index_] = min_distance;
      }
    }
  ...
  double getMinDistance(const Pose2D &path_point,
    ...
    return min_dis_result.distances[MultiModelMinDistances::FULL_BODY] <
                   min_dis_result.distances[MultiModelMinDistances::LOW_WHEELS]
               ? min_dis_result.distances[MultiModelMinDistances::FULL_BODY]
               : min_dis_result.distances[MultiModelMinDistances::LOW_WHEELS];
  }
  ...
  MultiModelMinDistances calcMinDistanceAndCompareWithGroundtruthDebug(
      const std::shared_ptr<MultiCircleFootprintModel> &footprint_model,
      const std::vector<SbpObstaclePtr> &obs_ptrs) {
    ...
    result = calcMultiModelMinDistance(footprint_model);

    double min_dis_full_body =
        result.distances[MultiModelMinDistances::FULL_BODY];
    double diff = min_dis_full_body - min_distance_gt;
