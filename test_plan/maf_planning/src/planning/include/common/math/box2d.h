...
#ifndef MODULES_PLANNING_MATH_BOX2D_H_
#define MODULES_PLANNING_MATH_BOX2D_H_
...
public:
  ...
  /**
   * @brief update box attributes with the center, heading, length and width.
   * @param center The center of the rectangular bounding box.
   * @param heading The angle between the x-axis and the heading-axis,
   *        measured counter-clockwise.
   * @param length The size of the heading-axis.
   * @param width The size of the axis perpendicular to the heading-axis.
   */
  void set_box(const Vec2d &center, const double heading, const double length,
               const double width);
  ...
  bool IsPointOnBoundarySoft(const Vec2d &point) const;
  ...
  /**
   * @brief Determines the distance between the box and a given point and judge
   * the left or right
   * @param point The point whose distance to the box we wish to compute
   * @return A distance and left or right
   */
  std::pair<int, double> DistanceTo(const Vec2d &point, bool diff) const;
  ...
  /**
   * @brief Get lateral distance from center axis
   * @return both positive and negative
   */
  double LateralDistanceTo(const LineSegment2d &line_segment) const;

  /**
   * @brief Get lateral distance from center axis
   * @return both positive and negative
   */
  double LateralDistanceTo(const Box2d &box) const;

  /**
   * @brief Get longitudinal distance from center axis
   * @return both positive and negative
   */
  double LongitudinalDistanceTo(const LineSegment2d &line_segment) const;

  /**
   * @brief Get longitudinal distance from center axis
   * @return both positive and negative
   */
  double LongitudinalDistanceTo(const Box2d &box) const;

  bool isCorrectLateralSide(const LineSegment2d &line_segment,
  ...
  NLOHMANN_DEFINE_TYPE_INTRUSIVE(Box2d, center_, length_, width_, half_length_,
                                 half_width_, heading_, cos_heading_,
                                 sin_heading_, corners_, max_x_, min_x_, max_y_,
                                 min_y_)
...
#endif /* MODULES_PLANNING_MATH_BOX2D_H_ */
