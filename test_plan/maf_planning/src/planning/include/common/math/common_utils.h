#ifndef MODULES_PLANNING_COMMON_UTIL_H_
#define MODULES_PLANNING_COMMON_UTIL_H_
...
/**
 * @namespace msquare::common_utils
 * @brief msquare::common::util
 */
namespace msquare {
namespace util {
...
template <typename T> planning_math::Vec2d MakeVec2d(const T &t) {
  return planning_math::Vec2d(t.x(), t.y());
}
...
template <typename T> void QuaternionToRotationMatrix(const T *quat, T *R) {
  T x2 = quat[0] * quat[0];
  T xy = quat[0] * quat[1];
  T rx = quat[3] * quat[0];
  T y2 = quat[1] * quat[1];
...
#endif /* MODULES_PLANNING_COMMON_UTIL_H_ */
