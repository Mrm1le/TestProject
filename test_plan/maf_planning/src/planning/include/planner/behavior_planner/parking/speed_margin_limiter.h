...
is_last_segment, update_wheel_stop, is_dynamic_planning, is_apa_not_parallel, speed_type, last_v, travel_s, last_delta_s, vec_sv)
...
struct SpeedMarginPara{
    ...
    void setDynamicPlanning(double _last_v, double _last_delta_s, bool _is_dynamic_planning){
...
public:
    SpeedMarginLimiter(){}
    SpeedMarginLimiter(const SpeedMarginPara& para, const std::vector<Pose2D>& traj, const std::vector<double>& curvatures, bool is_debug = false):
    ...
    bool filterByMinS(VecSV &vec_sv, const std::vector<unsigned int>& change_index);
    ...
    double getTotals();
    std::vector<std::vector<msquare::planning_math::Vec2d>> getTrajDebug(){return traj_debug_;}
