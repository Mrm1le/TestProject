...
public:
  ...
  void
  set_kappa_bound(const std::vector<std::pair<double, double>> &kappa_bounds);
  ...
  void set_x_ref(const std::vector<double> &weight_x_ref_vec,
                 const std::vector<double> &x_ref);
  void set_dx_ref(const std::vector<double> &weight_dx_ref_vec,
  ...
  void set_weight_kappa(const double weight_kappa,
...
private:
  ...
  void getGradientLbAndUbAndA(Eigen::VectorXd &lb, Eigen::VectorXd &ub,
                              Eigen::MatrixXd &A);
  void getInequalityConstraintOfXAndDdx(double *k, double *b,
