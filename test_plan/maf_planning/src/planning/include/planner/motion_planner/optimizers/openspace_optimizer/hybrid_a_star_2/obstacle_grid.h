...
public:
  ...
  bool constructFromSbpObstacle(
    ...
    for (std::size_t i = 0; i < discrete_obs.size(); i++) {
      ...
      printf("obs_points size %lu, origin_x_ %.3f, origin_y_ %.3f, inv_res_ "
             "%.3f\n",
  ...
  bool constructFromPointsInGrid(const std::vector<cv::Point2f> &points_in_grid,
    ...
    auto fill_polygon_timer =
        msd_planning::utils::Timer<true>("fill_convex_poly");
    ...
    printf("voroni_centers_buffer_ %lu, voroni_faces_buffer_ size %lu, "
           "voroni_faces_buffer_[0] size %lu\n",
           voroni_centers_buffer_.size(), voroni_faces_buffer_.size(),
           voroni_faces_buffer_.front().size());
    ...
    printf("grid_data_f32_buffer_rows %d cols %d\n", grid_data_f32_buffer_.rows,
           grid_data_f32_buffer_.cols);
    ...
    for (std::size_t i = 0; i < grid_data_u16_[(int)height].size(); i++) {
      if (i % 100000 == 0)
        printf("grid_data_u16_ size %lu\n grid_data_f32_buffer_.data %.3f "
               "grid_data_scale_ %.3f",
               grid_data_u16_[(int)height].size(), grid_data_ptr[i],
               grid_data_scale_);
