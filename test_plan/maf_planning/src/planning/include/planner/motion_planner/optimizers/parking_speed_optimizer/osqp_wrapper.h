...
class DataWrapper {
 public:
  ...
  void setConstraintsMatrix(const Eigen::Matrix<double, -1, -1>& constraintsMatrix);

  template <typename Derived>
  void setHessianSparseMatrix(const Eigen::SparseCompressedBase<Derived>& hessianMatrix);

  // template <typename Derived>
  // void setHessianMatrix(const Eigen::SparseCompressedBase<Derived>& hessianMatrix);

  template <typename Derived>
  void setConstraintsSparseMatrix(const Eigen::SparseCompressedBase<Derived>& constraintsMatrix);
...
class OsqpWrapper {
 public:
  OsqpWrapper();
  OsqpWrapper(const Eigen::Matrix<double, -1, -1>& H, const Eigen::Matrix<double, -1, 1>& g,
              const Eigen::Matrix<double, -1, -1>& A, const Eigen::Matrix<double, -1, 1>& l,
 ...
 public: //对外接口函数
  ...
  void updateGradient(const Eigen::Ref<const Eigen::Matrix<c_float, Eigen::Dynamic, 1>>& gradient);
  ...
  void updateBounds(const Eigen::Ref<const Eigen::Matrix<c_float, Eigen::Dynamic, 1>>& lowerBound,
                    const Eigen::Ref<const Eigen::Matrix<c_float, Eigen::Dynamic, 1>>& upperBound);

  template <typename T>
  bool updateHessianMatrix(const Eigen::SparseCompressedBase<T>& hessianMatrix);
  template <typename T>
  bool updateLinearConstraintsMatrix(const Eigen::SparseCompressedBase<T>& linearConstraintsMatrix);
 ...
 private:
  ...
  bool isValidConstraints(const Eigen::Index& n, const Eigen::MatrixXd& A, const Eigen::VectorXd& l,
                          const Eigen::VectorXd& u);
  bool isValidSetup(const Eigen::MatrixXd& H, const Eigen::VectorXd& g, const Eigen::MatrixXd& A,
                    const Eigen::VectorXd& l, const Eigen::VectorXd& u);

 private:                                                                          //成员变量　
  std::unique_ptr<OSQPWorkspace, std::function<void(OSQPWorkspace*)>> m_workspace; //自定义删除器
...
void DataWrapper::setConstraintsSparseMatrix(
  ...
  if (constraintsMatrix.rows() != m_data->m || constraintsMatrix.cols() != m_data->n) {
...
template <typename T>
bool OsqpWrapper::createOsqpSparseMatrix(const Eigen::SparseCompressedBase<T>& eigenSparseMatrix,
                                         csc*& osqpSparseMatrix) {
  // 此处typename告诉编译器value_type　为一个类型,采用列优先存储
  Eigen::SparseMatrix<typename T::value_type, Eigen::ColMajor> colMajorCopy = eigenSparseMatrix;
  ...
  const int32_t* const outerIndexPtr = colMajorCopy.outerIndexPtr(); //每一列非０元素个数
  ...
  for (int32_t k = 0; k < cols; k++) {
    ...
    for (typename Eigen::SparseMatrix<typename T::value_type, Eigen::ColMajor>::InnerIterator it(
             colMajorCopy, k);
         it; ++it) {
  ...
  osqpSparseMatrix->p[static_cast<int32_t>(cols)] = static_cast<c_int>(innerOsqpPosition);
  assert(innerOsqpPosition == numberOfNonZeroCoeff);
  return true;
}

template <typename T>
bool OsqpWrapper::updateHessianMatrix(const Eigen::SparseCompressedBase<T>& hessianMatrix) {
  if (!m_is_inited) {
...
bool OsqpWrapper::updateLinearConstraintsMatrix(
  ...
  if (static_cast<c_int>(linearConstraintsMatrix.rows()) != m_workspace->data->m ||
      static_cast<c_int>(linearConstraintsMatrix.cols()) != m_workspace->data->n) {
...
bool OsqpWrapper::setWarmStart(const Eigen::Matrix<T, n, 1>& primalVariable,
  ...
  osqp_warm_start(m_workspace.get(), primalVariable.data(), dualVariable.data());
  return true;
}
} // namespace parking
} // namespace msquare
