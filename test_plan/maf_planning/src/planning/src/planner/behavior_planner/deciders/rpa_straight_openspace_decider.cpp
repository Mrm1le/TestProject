...
void RpaStraightOpenspaceDecider::reset() {
...
void RpaStraightOpenspaceDecider::getOpenspaceBoundary() {
  ...
  double dis_init_target = std::hypot(
    local_target_pose_.x - local_init_pose_.x,
    local_target_pose_.y - local_init_pose_.y
  );

  double box_length = std::max(max_box_length, dis_init_target + VehicleParam::Instance()->length);
  double box_width = std::max(max_box_width, 4 * VehicleParam::Instance()->width);

  tlines_.emplace_back(
      msquare::planning_math::Vec2d( - box_width/ 2.0, VehicleParam::Instance()->width),
      msquare::planning_math::Vec2d( box_width/ 2.0, VehicleParam::Instance()->width)
  );
  tlines_.emplace_back(
      msquare::planning_math::Vec2d( - box_width/ 2.0, -VehicleParam::Instance()->width),
      msquare::planning_math::Vec2d( -VehicleParam::Instance()->length, -VehicleParam::Instance()->width)
  );
  tlines_.emplace_back(
      msquare::planning_math::Vec2d( -VehicleParam::Instance()->length, -2.0 * VehicleParam::Instance()->width),
      msquare::planning_math::Vec2d( -VehicleParam::Instance()->length, -VehicleParam::Instance()->width)
  );
  tlines_.emplace_back(
      msquare::planning_math::Vec2d( VehicleParam::Instance()->length, -2.0 * VehicleParam::Instance()->width),
...
bool RpaStraightOpenspaceDecider::getObstacles() {
  ...
  planning_math::Vec2d ego_back(ego_pose.x - VehicleParam::Instance()->back_edge_to_center * cs, ego_pose.y - VehicleParam::Instance()->back_edge_to_center * ss);
  planning_math::Vec2d ego_front(ego_pose.x + VehicleParam::Instance()->front_edge_to_center * cs, ego_pose.y + VehicleParam::Instance()->front_edge_to_center * ss);
  ...
  for (auto &parking_lot : parking_map_info.parking_lots_detection_fusion_results){
    ...
    lines_.push_back(ObstacleLine(stopper_line, OpenspaceObstacleType::PARKINGLINE));
  }
  ...
  for (auto &obj : world_model_->obstacle_manager().get_obstacles().Items()) {
    ...
    if (is_obstacle_vehicle) {
      ...
      if (is_object_considered ) {
        const planning_math::Box2d &obj_boundingbox = obj->PerceptionBoundingBox();
        boxes_.push_back(
  ...
  for(auto& l:tlines_){
    lines_.push_back(ObstacleLine(
      planning_math::tf2d_inv(local_frame_pose_, l),
