...
double SvSpeedGenerator::getSpeed(const planning_math::Box2d& slot_box, const Pose2D & ego_pose, const std::vector<float> &traj_curvature, const SvSpeedParam &sv_p, bool & is_in_slot){
    double velocity = regulateSpeed(slot_box, ego_pose, traj_curvature, sv_p, is_in_slot);
    if(sv_p.is_reverse){
        velocity = -velocity;
    }
    return velocity;
}
double SvSpeedGenerator::regulateSpeed(const planning_math::Box2d& slot_box, const Pose2D & ego_pose, const std::vector<float> &traj_curvature, const SvSpeedParam &sv_p, bool & is_in_slot){
    double velocity = 0.0;
    ...
    if(!sv_p.no_slot){
        ...
        planning_math::Box2d ego_box(ego_center, ego_pose.theta, sv_p.length, sv_p.width_wo_rearview_mirror);
