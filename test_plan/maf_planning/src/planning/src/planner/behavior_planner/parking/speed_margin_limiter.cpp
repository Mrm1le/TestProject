...
bool SpeedMarginLimiter::limitVAtStart(VecSV &vec_sv){
    ...
    // flat scenario
    while(cur_iter != vec_sv.end() && std::abs(cur_iter->second - last_iter->second)<eps){
            cur_iter ++;
    }
    if(cur_iter == vec_sv.end() || last_iter->second < eps || cur_iter->second < eps){
        // this should wont run here
        return false;
    }
    double flat_elapse_time = (last_iter->first - (cur_iter+1)->first) / last_iter->second;
...
bool SpeedMarginLimiter::filterByRemainS(VecSV &vec_sv){
    ...
    while(cur_iter != vec_sv.end()){
        ...
        if(cur_iter->second - last_iter->second > eps){
            LOCAL_LOG(LOCAL_DEBUG, "up scenario:");
            while(last_iter != vec_sv.begin() && cur_iter->second - last_iter->second > eps){
                last_iter --;
                cur_iter --;
            }
            break;
        ...
        while(last_iter != vec_sv.begin() && std::abs(cur_iter->second - last_iter->second)<eps &&  last_iter->first - cur_back_iter->first < flat_min_s){
            last_iter --;
            cur_iter --;
        }
        ...
        //  flat is enough
        if(std::abs(cur_iter->second - last_iter->second)<eps &&  last_iter->first - cur_back_iter->first > flat_min_s){
        ...
        last_iter --;
        cur_iter --;
        while(last_iter != vec_sv.begin() && std::abs(cur_iter->second - last_iter->second)>eps){
            last_iter --;
            cur_iter --;
        }
        break;
    ...
    std::vector<double> ss = {
        // para_.control_take_over_remain_s,
    ...
    for(int i=0;i<anchor.size();i++){
        ...
        while(cur_iter != vec_sv.end()){
            ...
            VecSV::iterator pose_iter = last_iter->first < anchor[i].first ? last_iter:cur_iter;
...
bool SpeedMarginLimiter::limitVByCurvature(VecSV &vec_sv){
    if(curvatures_.size() != vec_sv.size()){
        // MSD_LOG(ERROR, "limitVByCurvature:curvatures_.size()=%d, vec_sv.size()=%d",curvatures_.size(), vec_sv.size());
        return false;
    }
    ...
    for(unsigned int index=1;index<curvatures_.size();index++){
        ...
        if(delta_cur > para_.curvature_change_threshold){
            vec_sv[index].second = std::min(vec_sv[index].second, para_.curvature_limit_v);
        }
    }
    return true;
}
...
bool SpeedMarginLimiter::limitVByObstacle(VecSV &vec_sv_raw){
    ...
    for(auto &p: traj_){
        ...
        planning_math::Vec2d center_line_start(
            p.x + (para_.is_reverse ? para_.back_p1 * ego_cos : para_.front_p1 * ego_cos),
            p.y + (para_.is_reverse ? para_.back_p1 * ego_sin : para_.front_p1 * ego_sin)
        );
        planning_math::Vec2d center_line_end(
            p.x + (para_.is_reverse ? para_.back_p2 * ego_cos : para_.front_p2 * ego_cos),
            p.y + (para_.is_reverse ? para_.back_p2 * ego_sin : para_.front_p2 * ego_sin)
        );
        ...
        if(is_debug_){
            std::vector<msquare::planning_math::Vec2d> corners = ego_box.GetAllCorners();
            traj_debug_.push_back(corners);
        }
        planning_math::Box2d ego_box_large(
            planning_math::LineSegment2d(center_line_start, center_line_end),
            para_.width + 2 * std::max(para_.obstacle_consider_max, para_.mirror_extrude_length + para_.side_add)
        );
        planning_math::Vec2d mirror_center_line_start(
            p.x + para_.mirror_p1 * ego_cos,
            p.y + para_.mirror_p1 * ego_sin
        );
        planning_math::Vec2d mirror_center_line_end(
            p.x + para_.mirror_p2 * ego_cos,
            p.y + para_.mirror_p2 * ego_sin
        );
        planning_math::Box2d mirror_ego_box(
            planning_math::LineSegment2d(mirror_center_line_start, mirror_center_line_end),
        ...
        //     }
        // }
        // MSD_LOG(ERROR, "limitVByCurvature: box obstacle min_dis = %.3f", min_dis);
        for(auto &obs_p:para_.obs_pts){
            if(min_dis < 1e-6){break;}
            double dis_to_center = planning_math::sum_square(obs_p.x() - p.x, obs_p.y() - p.y);
...
bool SpeedMarginLimiter::getVConsiderLast(double s, double last_v,  double& v, SpeedRes & res){
    ...
    if(!para_.is_reverse || !para_.is_apa_not_parallel || !para_.is_last_segment){
        return is_valid;
    }
...
bool SpeedMarginLimiter::adaptDynamicPlanning(VecSV &vec_sv){
    ...
    while(cur_iter != vec_sv.end()){
        ...
        if(std::abs(cur_iter->second - para_.last_v)<eps){
            ...
            vec_sv.insert(vec_sv.begin(), {last_iter->first + traveled_s + delta_s, para_.last_v});
            break;
        }
        ...
        double dec_s = (para_.last_v * para_.last_v - temp_v * temp_v) / (2 * para_.remain_s_planning_deceleration);
        ...
        while(cur_iter >= last_iter){
            ...
            if(last_to_temp_s > dec_end_s){
                ...
                cur_iter->second = std::sqrt(last_v_square - 2 * para_.remain_s_planning_deceleration * dec_temp_s);
            }else{
        ...
        if(para_.last_res.type == CONST_SPEED){
            VecSV temp_vec = {{last_iter->first + traveled_s +  delta_s, para_.last_v}, {last_iter->first + delta_s, para_.last_v}};
...
bool SpeedMarginLimiter::getV(double s, double& v, SpeedRes & res){
    ...
    if(std::abs(vec_sv_[left].second - vec_sv_[left-1].second) < eps && std::abs(v - vec_sv_[left-1].second) < eps){
        res.type = SpeedType::CONST_SPEED;
    }

    while(left>1 && std::abs(vec_sv_[left - 2].second - vec_sv_[left-1].second) < eps){
            left--;
    }
    res.travel_s = (res.type == SpeedType::CONST_SPEED ? std::abs(vec_sv_[left-1].first -s): 0.0);
    
    return true;
}

bool SpeedMarginLimiter::filterByMinS(VecSV &vec_sv, const std::vector<unsigned int>& change_index){
    ...
    while(next_i < v_step_list.size()){
        ...
        // different v
        double a = delta_v >0.0 ? para_.acc : -para_.remain_s_planning_deceleration;
...
std::vector<unsigned int> SpeedMarginLimiter::filterByMinV(double window_size, VecSV &vec_sv){
    ...
    while(iter_f != vec_sv.end()){
        if(iter_min < iter_f){
            ...
            iter_min = std::min_element(iter_f, iter_b, [](const SV_PAIR& sv0,const SV_PAIR& sv1){
                return sv0.second < sv1.second;
            });
        }

        while(iter_b != vec_sv.end() && std::abs(iter_b->first - iter_f->first) < window_size){
            // find the min v
        ...
        if(iter_min != last_iter_min){
            if(last_iter_min == vec_sv.end() || std::abs(iter_min->second - last_iter_min->second)>1e-3 || (iter_min + 1) == vec_sv.end()){
...
double SpeedMarginLimiter::getVByRemains(double remain_s){
    ...
    if(remain_s > s1){
        ...
        v_target = std::sqrt(para_.control_take_over_speed * para_.control_take_over_speed + 2 * para_.remain_s_planning_deceleration * delta_s);
    }else if(remain_s > s3){
        double delta_s = remain_s - s3;
        v_target = std::sqrt(para_.remain_s_min_velocity * para_.remain_s_min_velocity + 2 * para_.control_take_over_acc * delta_s);;
...
void SpeedMarginPara::init(bool _is_reverse, bool _is_last_segment,
    ...
    double front_edge_to_mirror = msquare::CarParams::GetInstance()->car_config.car_only_config.front_edge_to_mirror;
    double mirror_length = msquare::CarParams::GetInstance()->car_config.car_only_config.mirror_length;
    double back_edge_to_center = msquare::VehicleParam::Instance()->back_edge_to_center;
    double front_edge_to_center = msquare::VehicleParam::Instance()->front_edge_to_center;
    obstacle_consider_max = msquare::CarParams::GetInstance()->car_config.lon_config.obstacle_consider_max;
    double obstacle_consider_min = msquare::CarParams::GetInstance()->car_config.lon_config.obstacle_consider_min;
    width_wo_rearview_mirror = msquare::VehicleParam::Instance()->width_wo_rearview_mirror;
    width = msquare::VehicleParam::Instance()->width;
    length = msquare::VehicleParam::Instance()->length;
    obstacle_consider_extend_length = msquare::CarParams::GetInstance()->car_config.lon_config.obstacle_consider_extend_length;
    ...
    obstacle_consider_min_v = msquare::CarParams::GetInstance()->car_config.lon_config.obstacle_consider_min_v;
    max_v = is_reverse ? msquare::TrajectoryOptimizerConfig::GetInstance()->param_max_speed_reverse : msquare::TrajectoryOptimizerConfig::GetInstance()->param_max_speed_forward;
    obstacle_consider_max = msquare::CarParams::GetInstance()->car_config.lon_config.obstacle_consider_max;
    v_slop = (max_v - obstacle_consider_min_v)/(std::max(obstacle_consider_max-obstacle_consider_min, 0.1));

    window_s_size = msquare::CarParams::GetInstance()->car_config.lon_config.window_s_size;
    acc = msquare::CarParams::GetInstance()->car_config.lon_config.acc;
    dec = msquare::CarParams::GetInstance()->car_config.lon_config.dec;
    limit_dec = msquare::CarParams::GetInstance()->car_config.lon_config.limit_dec;
    dt = 1.0 / msquare::CarParams::GetInstance()->car_config.lon_config.rate;
    forbid_acc_delta_v = limit_dec * dt;
    forbid_acc_max_s = msquare::CarParams::GetInstance()->car_config.lon_config.forbid_acc_max_s;

    min_duration_filter_time = msquare::CarParams::GetInstance()->car_config.lon_config.min_duration_filter_time;
    curvature_change_threshold = msquare::CarParams::GetInstance()->car_config.lon_config.curvature_change_threshold;
    curvature_limit_v = msquare::CarParams::GetInstance()->car_config.lon_config.curvature_limit_v;
    
    traj_end_speed_limit_s = msquare::CarParams::GetInstance()->car_config.lon_config.traj_end_speed_limit_s;
    control_take_over_remain_s = msquare::CarParams::GetInstance()->car_config.lon_config.control_take_over_remain_s;
    control_take_over_speed = msquare::CarParams::GetInstance()->car_config.lon_config.control_take_over_speed;
    control_take_over_acc = msquare::CarParams::GetInstance()->car_config.lon_config.control_take_over_acc;
    remain_s_min_velocity = msquare::CarParams::GetInstance()->car_config.lon_config.min_velocity;
    remain_s_planning_deceleration = msquare::CarParams::GetInstance()->car_config.lon_config.planning_deceleration;
    if(is_last_segment){
        remain_s_planning_deceleration = msquare::CarParams::GetInstance()->car_config.lon_config.planning_deceleration_last;
    }
    const_speed_min_s = std::abs(max_v * max_v - control_take_over_speed * control_take_over_speed) / (2 * remain_s_planning_deceleration) + control_take_over_remain_s;
    lower_const_speed_min_s = control_take_over_remain_s - (control_take_over_speed * control_take_over_speed - remain_s_min_velocity * remain_s_min_velocity) / ( 2 * control_take_over_acc);
}
...
SpeedMarginDebug SpeedMarginLimiter::generateFromString(const std::string& info){
