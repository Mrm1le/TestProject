...
bool ParkingLongitudinalMotionPlanner::computeVelV2(
  ...
  double min_finish_len = msquare::CarParams::GetInstance()
                              ->car_config.common_config.min_finish_len;
  ...
  if ((PlanningContext::Instance()
    ...
    v_target_ = 0.0;
    MSD_LOG(ERROR,
            "gear changing is: %d ,  real gear is: %d , cur gear is %d, "
            "traj_length is: %f",
  ...
  compute_speed_for_remain_distanceV2(lead_cars, freespace, fs_line,
                                      is_reverse);
  ...
  MSD_LOG(ERROR, "limit_speed_for_margin_limit: before v_target_ = %.3f",
          v_target_);

  limit_speed_for_margin_limit();
  MSD_LOG(ERROR, "limit_speed_for_margin_limit: v_target_ = %.3f", v_target_);

  compute_speed_use_osqp(lead_cars, freespace, fs_line, is_reverse,
                         is_stop_status);
  if (PlanningContext::Instance()
  ...
  MSD_LOG(ERROR, "compute_speed_for_prediction_obstacle: v_target_ = %.3f",
          v_target_);
  if (is_openspace) {
...
bool ParkingLongitudinalMotionPlanner::checkBlockV2(
  ...
  //   remaining_distance = std::min(lead_one.d_rel - lon_inflation_min,
  //   remaining_distance);
  // }
...
bool ParkingLongitudinalMotionPlanner::compute_speed_with_leadsV2(
  ...
  if (lead_cars.first.id != -1 && lead_cars.first.is_static &&
      lead_cars.first.type == ObjectType::COUPE) {
  ...
  if (lead_cars.first.id != -1) {
    MSD_LOG(ERROR, "lead_cars.first.d_rel = %.3f id: %d", lead_cars.first.d_rel,
            lead_cars.first.id);
    ...
    if (lead_cars.second.id != -1) {
      MSD_LOG(ERROR, "lead_cars.second.d_rel = %.3f id: %d",
              lead_cars.second.d_rel, lead_cars.first.id);
  ...
  MSD_LOG(ERROR, "leads v_target_= %.3f, v_target_lead = %.3f", v_target_,
          v_target_lead);
...
void ParkingLongitudinalMotionPlanner::compute_speed_for_remain_distanceV2(
  ...
  if (lead_point.id != -1) {
    ...
    MSD_LOG(ERROR, "lead point remain_s is: %f, the pos is:x %f, y %f",
            min_obs_s, lead_point.x, lead_point.y);
  }

  // if (lead_cars.first.id != -1 && !lead_cars.first.is_static) {
  //   n_des = std::max(0.0, lead_cars.first.d_rel);
  //   min_obs_s = min_obs_s > n_des ? n_des : min_obs_s;
  //   MSD_LOG(ERROR, "first lead car remain_s is: %f, id is:%d", min_obs_s,
  //   lead_cars.first.id); if (lead_cars.second.id != -1 &&
  //   !lead_cars.second.is_static) {
  //     n_des = std::max(0.0, lead_cars.first.d_rel);
  //     min_obs_s = min_obs_s > n_des ? n_des : min_obs_s;
  //     MSD_LOG(ERROR, "second lead car remain_s is: %f, id is:%d", min_obs_s,
  //     lead_cars.second.id);
  //   }
  // }
  ...
  if (fs_line.id != -1) {
    ...
    min_obs_s = min_obs_s > n_des ? n_des : min_obs_s;
    MSD_LOG(ERROR,
            "first lead line remain_s is: %f, the pos is:x %f, y %f, x_end:%f, "
            "y_end:%f",
            min_obs_s, fs_line.x_start, fs_line.y_start, fs_line.x_end,
            fs_line.y_end);
  }
  ...
  double consider_obstacle_max_s =
      msquare::CarParams::GetInstance()
          ->car_config.lon_config.consider_obstacle_max_s;
  ...
  double start_speed_reduce_remain_s =
      std::abs(max_forward_speed * max_forward_speed -
               control_take_over_speed * control_take_over_speed) /
          (2 * planning_deceleration) +
  ...
  if (PlanningContext::Instance()
    ...
    if (remain_s < traj_end_speed_limit_s) {
      ...
      if (delta_s > control_take_over_s) {
        ...
        if (remain_s > traj_end_speed_limit_s &&
            remain_s < traj_end_speed_limit_s + diff_s) {
          ...
          v_target_ =
              std::min(std::sqrt(min_velocity * min_velocity +
                                 2 * control_take_over_acc * speed_low_diff_s),
                       v_target_);
        }
      ...
      v_target_ =
          std::min(std::sqrt(control_take_over_speed * control_take_over_speed +
                             2 * planning_deceleration * delta_s),
                   v_target_);
...
bool ParkingLongitudinalMotionPlanner::limit_speed_for_margin_limit() {
  ...
  if (path_segment_updated) {
    ...
    MSD_LOG(ERROR, "update path segment: receive updated traj_size=%d",
            plan_traj.size());
    int cut_num = 10;
    if (plan_traj.size() < cut_num + 2) {
      return false;
    }
    if (plan_traj.size() != curvatures.size()) {
      return false;
    }
  ...
  double limit_v;
  if (!speed_margin_limiter_.getVConsiderLast(traj_length, last_margin_v_,
                                              limit_v, last_res_)) {
    last_margin_v_ = -1.0;
...
bool ParkingLongitudinalMotionPlanner::marginVelocityLimit(
    const Pose2DTrajectory &plan_traj, const std::vector<double> &curvatures) {
  ...
  bool is_reverse = (planning_result.gear == GearState::REVERSE);
  const bool is_not_parallel =
      instance->parking_behavior_planner_output()
          .parking_slot_info.type.value != ParkingSlotType::PARALLEL;
  const bool is_parkin =
      instance->planning_status().scenario.status_type == StatusType::APA;
  const bool is_at_last_segment =
      instance->longitudinal_behavior_planner_output().is_at_last_segment;
  ...
  speed_margin_para.init(is_reverse, is_at_last_segment, obs_boxes, obs_pts,
                         line_obs_final);
  speed_margin_para.is_apa_not_parallel = (is_not_parallel && is_parkin);
  bool update_wheel_stop =
      instance->longitudinal_behavior_planner_output().update_wheel_stop;
  bool is_dynamic_planning =
      instance->longitudinal_behavior_planner_output().is_dynamic_planning;
  ...
  speed_margin_para.setDynamicPlanning(last_margin_v_, last_delta_s,
                                       is_dynamic_planning);
  speed_margin_para.last_res = last_res_;
  MSD_LOG(ERROR, "is_dynamic_planning = %d, update_wheel_stop = %d",
          is_dynamic_planning, update_wheel_stop);
  speed_margin_limiter_ =
      SpeedMarginLimiter(speed_margin_para, plan_traj, curvatures, false);
...
double ParkingLongitudinalMotionPlanner::compute_remain_distance_for_qp(
  ...
  MSD_LOG(ERROR, "init traj remain_s is: %f, min_obs is: %f", remain_s,
          min_obs_s);
  ...
  MSD_LOG(ERROR, "min_remain_v: %f, traj_length_margin_v: %f ", min_remain_v,
          traj_length_margin_v);
...
void ParkingLongitudinalMotionPlanner::compute_speed_use_osqp(
    const LeaderPair &lead_cars, const FreespacePoint &lead_point,
    const FreespaceLine &fs_line, bool is_reverse, bool is_stop_status) {
  const auto &planner_output =
      PlanningContext::Instance()->longitudinal_behavior_planner_output();
  PlanningResult &planning_result =
      PlanningContext::Instance()->mutable_planning_status()->planning_result;
  std::vector<TrajectoryPoint> pre_pwj_trajectory =
      planning_result.pwj_trajectory;
  ...
  double remain_s =
      compute_remain_distance_for_qp(lead_cars, lead_point, fs_line);
  ...
  const bool isAPA = (PlanningContext::Instance()
                          ->parking_behavior_planner_output()
                          .planner_type == PlannerType::OPENSPACE);
  msquare::parking::VecST first_st_obs;
  msquare::parking::VecST second_st_obs;
  computeSTFromODObs(lead_cars, ego_speed, isAPA, first_st_obs, second_st_obs,
                     config.dt);
  // std::cout << "the first_st_obs size is: " << first_st_obs.size() <<
  // std::endl;
  ...
  double total_path_length = PlanningContext::Instance()
                                 ->longitudinal_behavior_planner_output()
                                 .traj_length;
  double init_a = 0.0;
  double init_speed =
      getInitSpeedForQP(speed_data, std::abs(ego_speed), &init_a);
  piece_wise_optimizer.makeOptimize(init_speed, init_a, &path, fix_time_vec_st,
                                    first_st_obs, second_st_obs);
...
bool ParkingLongitudinalMotionPlanner::compute_speed_for_apaV2() {
  ...
  if (parking_behavior_planner_output.is_move_ready) {
    ...
    //   MSD_LOG(INFO, "index & SIZE: %d %ld %ld", index,
    //   traj_vel_array_.size(),
...
bool ParkingLongitudinalMotionPlanner::computeSTFromODObs(
  ...
  double dist_to_poi = world_model_->get_distance_to_poi();
  if (lead_cars.first.id != -1) {
    // std::cout << "---------------> first lead car and the v is: " <<
    // lead_cars.first.v_lon  << std::endl;
    a_lead_p = process_a_lead(lead_cars.first.a_lon);
    for (double t = 0; t < 3.0; t = t + delta_t) {
      double temp_t = 1.0;
      d_des = calc_desired_distance(
          lead_cars.first.v_lon, v_ego,
          isAPA || lead_cars.first.is_sidepass_obj || (dist_to_poi < 0.0),
          false, lead_cars.first.is_static, IntentionObsType::NONE);
      v_target_lead = calc_desired_speed(lead_cars.first.d_rel, d_des,
                                         lead_cars.first.v_lon, a_lead_p);
      // std::cout << "the first obs t is: " << t << " " << v_target_lead <<
      // std::endl; first_st_obs.emplace_back(t, 0.0, v_target_lead, 0.0, 0.0);
...
void ParkingLongitudinalMotionPlanner::compute_speed_for_curvature() {
  ...
  for (size_t i = 0; i < traj_curvature.size() - 1; i++) {
    ...
    if (traj_curvature[i] * traj_curvature[i + 1] < 0 &&
      ...
      curvature_change_scope.emplace_back(traj_relative_s[i] - 0.3,
                                          traj_relative_s[i] + 0.3);
    }
  }
  ...
  auto get_current_s = [&ego_pose, &traj_pose, &traj_relative_s]() -> double {
    ...
    for (size_t i = 0; i < traj_pose.size(); i++) {
      if (std::hypot(traj_pose[i].x - ego_pose.x, traj_pose[i].y - ego_pose.y) <
          nearest_distance) {
  ...
  for (const auto scope : curvature_change_scope) {
    // std::cout << "the scope first:" << scope.first << " " << scope.second <<
    // std::endl;
...
void ParkingLongitudinalMotionPlanner::set_planning_resultV2(
    const double v_ego) {
  ...
  if (std::abs(v_target_) > std::abs(current_v_) ||
      remaining_distance_ < control_take_over_remain_s ||
      std::abs(current_v_ - v_target_) < 1e-2) {
...
double ParkingLongitudinalMotionPlanner::getInitSpeedForQP(
    const SpeedData &speed_data, double ego_speed, double *init_a) {
  ...
  auto pre_planning_time = PlanningContext::Instance()
                               ->planning_status()
                               .planning_result.pre_planning_time;
  auto cur_time = MTIME()->timestamp().us();
  double delta_time = double(cur_time - pre_planning_time) / 1000000;
  if (pre_planning_time < 0.1) {
    delta_time = 0.0;
  }
  speed_data.EvaluateByTime(speed_data.front().t + delta_time + 0.05,
                            &target_speed_data);
  MSD_LOG(ERROR, "the ego speed is: %f, the target v is:%f, the target a is:%f",
          ego_speed, target_speed_data.v, target_speed_data.a);
  if (std::fabs(ego_speed) < 0.1) {
    // std::cout << "the ego velocity has big error with target velocity, replan
    // speed planner" << std::endl;
  ...
  // target_speed = target_speed_data.v;
  *init_a = target_acc;
  MSD_LOG(ERROR, "final target v is:%f, final target a is:%f", target_speed,
          *init_a);
  return target_speed;
}
