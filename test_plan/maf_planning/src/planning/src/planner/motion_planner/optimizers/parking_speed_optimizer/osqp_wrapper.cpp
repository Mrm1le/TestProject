...
void DataWrapper::setGradient(Eigen::Matrix<c_float, Eigen::Dynamic, 1>& gradientVector) {
  if (gradientVector.rows() != m_data->n) {
    //PERROR << "The size of gradientVector is not equal to number of variables.";
    return;
  }
...
void DataWrapper::setLowerBound(Eigen::Matrix<c_float, Eigen::Dynamic, 1>& lowerVector) {
  if (lowerVector.rows() != m_data->m) {
    //PERROR << "The size of lowerVector is not equal to the number of constraints.";
    return;
  }
  m_isLowerBoundSet = true;
  m_data->l = lowerVector.data();
}
void DataWrapper::setUpperBound(Eigen::Matrix<c_float, Eigen::Dynamic, 1>& upperVector) {
...
bool DataWrapper::isSet() const {
  return m_isNumberOfVariablesSet && m_isLinearConstraintsMatrixSet && m_isHessianMatrixSet &&
         m_isGradientSet && m_isLinearConstraintsMatrixSet && m_isLowerBoundSet &&
         m_isUpperBoundSet;
}
OSQPData* const& DataWrapper::dataPtr() const { return m_data; }

void OsqpWrapper::OsqpWorkSpaceDeleter(OSQPWorkspace* ptr) noexcept {
  if (ptr != nullptr) osqp_cleanup(ptr);
}

OsqpWrapper::OsqpWrapper()
    : m_workspace(nullptr, OsqpWrapper::OsqpWorkSpaceDeleter), m_is_inited(false) {
  m_settings = std::unique_ptr<OSQPSettings>(new OSQPSettings()); /* default settings */
...
bool OsqpWrapper::clearVariables() {
  if (!m_is_inited) {
    //PERROR << "Unable to clear OSQP Wrapper. OSQP Wrapper Is Not Initialized";
    return false;
  }
...
const std::unique_ptr<OSQPWorkspace, std::function<void(OSQPWorkspace*)>>& OsqpWrapper::workspace()
    const {
  return m_workspace;
}
const std::unique_ptr<DataWrapper>& OsqpWrapper::data() const { return m_data; }
const std::unique_ptr<OSQPSettings>& OsqpWrapper::settings() const { return m_settings; }
...
void DataWrapper::setHessianMatrix(const Eigen::Matrix<double, -1, -1>& hessianMatrix) {
  if (!m_isNumberOfVariablesSet) {
    //PERROR << "Number Of Variables Not Set.";
    return;
  }
  if (hessianMatrix.rows() != dataPtr()->n || hessianMatrix.cols() != dataPtr()->n) {
...
void DataWrapper::setConstraintsMatrix(const Eigen::Matrix<double, -1, -1>& constraintsMatrix) {
  if (!m_isNumberOfConstraintsSet) {
    //PERROR << "Number Of Constraints Not Set.";
    return;
  }
  if (constraintsMatrix.rows() != dataPtr()->m || constraintsMatrix.cols() != dataPtr()->n) {
    //PERROR << "Input constraintsMatrix Size UnValid.";
    return;
  }
  Eigen::SparseMatrix<double> const sparse_constraints = constraintsMatrix.sparseView();
  setConstraintsSparseMatrix(sparse_constraints);
}

bool OsqpWrapper::isValidHessian(const Eigen::MatrixXd& H, const Eigen::VectorXd& g) {
...
bool OsqpWrapper::isValidConstraints(const Eigen::Index& n, const Eigen::MatrixXd& A,
...
bool OsqpWrapper::isValidSetup(const Eigen::MatrixXd& H, const Eigen::VectorXd& g,
...
OsqpWrapper::OsqpWrapper(const Eigen::Matrix<double, -1, -1>& H,
  ...
  m_workspace = std::unique_ptr<OSQPWorkspace, std::function<void(OSQPWorkspace*)>>(
...
OsqpWrapper::OsqpWrapper(const Eigen::Matrix<double, -1, -1>& H,
  ...
  OSQPWorkspace* OsqpWrapper::getOSQPWorkspace(const Eigen::Matrix<double, -1, -1>& H,
                         const Eigen::Matrix<double, -1, 1>& g,
                         const Eigen::Matrix<double, -1, -1>& A,
                         const Eigen::Matrix<double, -1, 1>& l,
                         const Eigen::Matrix<double, -1, 1>& u) {
