...
ReedShepp::ReedShepp() {
  max_kappa_ =
      std::tan(CarParams::GetInstance()->max_delta_angle / 180.0 * M_PI) /
...
ReedShepp::ReedShepp(double max_kappa, double step_size)
    : max_kappa_(max_kappa), step_size_(step_size) {
  all_possible_paths_.resize(paths_max_size_);
}

std::pair<double, double>
ReedShepp::calc_tau_omega(const double u, const double v, const double xi,
                          const double eta, const double phi) {
  double delta = planning_math::NormalizeAngle(u - v);
  ...
  double tau = 0.0;
  if (t2 < 0) {
    tau = planning_math::NormalizeAngle(t1 + M_PI);
  } else {
    tau = planning_math::NormalizeAngle(t1);
  }
  double omega = planning_math::NormalizeAngle(tau - u + v - phi);
  return std::make_pair(tau, omega);
}

bool ReedShepp::ShortestRSP(const std::shared_ptr<SearchNode> start_node,
                            const std::shared_ptr<SearchNode> end_node,
                            std::shared_ptr<ReedSheppPath> optimal_path) {
  return ShortestRSP(start_node->x, start_node->y, start_node->theta,
                     end_node->x, end_node->y, end_node->theta, optimal_path);
}

bool ReedShepp::ShortestRSP(double start_x, double start_y, double start_theta,
                            double end_x, double end_y, double end_theta,
                            std::shared_ptr<ReedSheppPath> optimal_path) {
  paths_size_ = 0;
  if (!GenerateRSP(start_x, start_y, start_theta, end_x, end_y, end_theta)) {
  ...
  if (!GenerateLocalConfigurations(start_x, start_y, start_theta, end_x, end_y,
                                   end_theta, optimal_path.get())) {
    // std::cout << "Fail to generate local configurations(x, y, phi) in
    // SetRSP";
    return false;
  }

  if (std::abs((*optimal_path).x.back() - end_x) > 1e-3 ||
      std::abs((*optimal_path).y.back() - end_y) > 1e-3 ||
      std::abs((*optimal_path).phi.back() - (end_theta)) > 1e-3) {
    ...
    // std::cout << "end x, y, phi are: " << end_x << ", "
    //        << end_y << ", " << end_theta;
    return false;
  }
...
bool ReedShepp::AllRSPs(const std::shared_ptr<SearchNode> start_node,
                        const std::shared_ptr<SearchNode> end_node,
                        std::vector<ReedSheppPath> *all_possible_path) {
  return AllRSPs(start_node->x, start_node->y, start_node->theta, end_node->x,
                 end_node->y, end_node->theta, all_possible_path);
}

bool ReedShepp::AllRSPs(double start_x, double start_y, double start_theta,
                        double end_x, double end_y, double end_theta,
  ...
  for (size_t i = 0; i < paths_size_; ++i) {
    bool is_valid = true;
    if (!GenerateLocalConfigurations(start_x, start_y, start_theta, end_x,
                                     end_y, end_theta,
                                     &(all_possible_paths_[i]))) {
      // std::cout << "Fail to generate local configurations(x, y, phi) in
      // SetRSP"
      //           << std::endl;
      is_valid = false;
    ...
    if (std::abs(all_possible_paths_[i].x.back() - end_x) > 1e-3 ||
        std::abs(all_possible_paths_[i].y.back() - end_y) > 1e-3 ||
        std::abs(all_possible_paths_[i].phi.back() - (end_theta)) > 1e-3) {
...
bool ReedShepp::GenerateRSP(double start_x, double start_y, double start_theta,
                            double end_x, double end_y, double end_theta) {
  double dx = end_x - start_x;
  double dy = end_y - start_y;
  double dphi = (end_theta - start_theta);
  double c = std::cos(start_theta);
  double s = std::sin(start_theta);
...
void ReedShepp::LSL(const double x, const double y, const double phi,
                    RSPParam *param) {
  std::pair<double, double> polar = planning_math::Cartesian2Polar(
      x - std::sin(phi), y - 1.0 + std::cos(phi));
  double u = polar.first;
  double t = polar.second;
  double v = 0.0;
  if (t >= 0.0) {
    v = planning_math::NormalizeAngle(phi - t);
    if (v >= 0.0) {
      param->flag = true;
      param->u = u;
      param->t = t;
...
void ReedShepp::LSR(const double x, const double y, const double phi,
  ...
  if (u1 >= 4.0) {
    ...
    t = planning_math::NormalizeAngle(t1 + theta);
    v = planning_math::NormalizeAngle(t - phi);
    if (t >= 0.0 && v >= 0.0) {
      param->flag = true;
      param->u = u;
      param->t = t;
...
void ReedShepp::LRL(const double x, const double y, const double phi,
  ...
  if (u1 <= 4.0) {
    ...
    t = planning_math::NormalizeAngle(t1 + 0.5 * u + M_PI);
    v = planning_math::NormalizeAngle(phi - t + u);
    if (t >= 0.0 && u <= 0.0) {
      param->flag = true;
      param->u = u;
      param->t = t;
...
void ReedShepp::SLS(const double x, const double y, const double phi,
                    RSPParam *param) {
  double phi_mod = planning_math::NormalizeAngle(phi);
  double xd = 0.0;
  double u = 0.0;
  double t = 0.0;
  double v = 0.0;
...
void ReedShepp::LRSR(const double x, const double y, const double phi,
  ...
  double xi = x + std::sin(phi);
  double eta = y - 1.0 - std::cos(phi);
  std::pair<double, double> polar = planning_math::Cartesian2Polar(-eta, xi);
  ...
  if (rho >= 2.0) {
    t = theta;
    u = 2.0 - rho;
    v = planning_math::NormalizeAngle(t + 0.5 * M_PI - phi);
...
void ReedShepp::LRSL(const double x, const double y, const double phi,
  ...
  double xi = x - std::sin(phi);
  double eta = y - 1.0 + std::cos(phi);
  std::pair<double, double> polar = planning_math::Cartesian2Polar(xi, eta);
  double rho = polar.first;
  double theta = polar.second;
  double r = 0.0;
  double t = 0.0;
  ...
  if (rho >= 2.0) {
    ...
    u = 2.0 - r;
    t = planning_math::NormalizeAngle(theta + std::atan2(r, -2.0));
    v = planning_math::NormalizeAngle(phi - 0.5 * M_PI - t);
    if (t >= 0.0 && u <= 0.0 && v <= 0.0) {
      param->flag = true;
      param->u = u;
      param->t = t;
...
void ReedShepp::LRSLR(const double x, const double y, const double phi,
  ...
  if (rho >= 2.0) {
    ...
    if (u <= 0.0) {
      t = planning_math::NormalizeAngle(
...
bool ReedShepp::GenerateLocalConfigurations(double start_x, double start_y,
  ...
  std::vector<int> pgear(point_num, 1);
  int index = 1;
  double d = 0.0;
  ...
  for (size_t i = 0; i < px.size(); ++i) {
    shortest_path->x.emplace_back(std::cos(-start_theta) * px.at(i) +
                                  std::sin(-start_theta) * py.at(i) + start_x);
    shortest_path->y.emplace_back(-std::sin(-start_theta) * px.at(i) +
                                  std::cos(-start_theta) * py.at(i) + start_y);
    shortest_path->phi.emplace_back(
        planning_math::NormalizeAngle(pphi.at(i) + start_theta));
  }
