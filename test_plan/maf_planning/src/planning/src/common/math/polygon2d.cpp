...
Polygon2d::Polygon2d(std::vector<Vec2d> &points) : points_(points) {
  (void)BuildFromPoints();
}

void Polygon2d::update(const Box2d &box) {
  box.GetAllCorners(&points_);
  (void)BuildFromPoints();
}

void Polygon2d::update(const std::vector<Vec2d> &points) {
  points_ = points;
  (void)BuildFromPoints();
}

double Polygon2d::DistanceTo(const Vec2d &point) const {
  mph_assert(points_.size() >= 3);
  if (points_.size() < 3) {
...
bool Polygon2d::Contains(const LineSegment2d &line_segment) const {
  ...
  if (!is_convex_) {
    static std::vector<LineSegment2d> overlaps;
    overlaps = GetAllOverlaps(line_segment);
...
bool Polygon2d::BuildFromPoints() {
  ...
  area_ /= 2.0;
  // mph_assert(area_ >= 0.0);
  mph_assert(!std::isnan(area_));
...
bool Polygon2d::ComputeConvexHull(const std::vector<Vec2d> &points,
  ...
  static std::vector<Vec2d> result_points;
...
bool Polygon2d::ClipConvexHull(const LineSegment2d &line_segment,
  ...
  static std::vector<Vec2d> new_points;
  new_points.clear();
  for (size_t i = 0; i < n; ++i) {
    if (side[i] >= 0) {
...
bool Polygon2d::ComputeOverlap(const Polygon2d &other_polygon,
  ...
  mph_assert(points_.size() >= 3);
  mph_assert(overlap_polygon != nullptr);
  mph_assert(is_convex_ && other_polygon.is_convex());
  if (points_.size() < 3 || overlap_polygon == nullptr || !is_convex_ ||
      !other_polygon.is_convex()) {
    return false;
  }

  // debug info to be discarded
  for (auto point : other_polygon.points()) {
    mph_assert(!std::isnan(point.x()) && !std::isnan(point.y()));
  }
...
bool Polygon2d::GetOverlap(const LineSegment2d &line_segment,
  ...
  mph_assert(points_.size() >= 3);
  mph_assert(first != nullptr);
  mph_assert(last != nullptr);
  if (points_.size() < 3 || first == nullptr || last == nullptr) {
    return false;
  }
...
Polygon2d::GetAllOverlaps(const LineSegment2d &line_segment) const {
  static std::vector<LineSegment2d> overlap_line_segments;
  overlap_line_segments.clear();
  mph_assert(points_.size() >= 3);
  if (points_.size() < 3) {
  ...
  static std::vector<double> projections;
  projections.clear();
  if (IsPointIn(line_segment.start())) {
  ...
  static std::vector<std::pair<double, double>> overlaps;
  overlaps.clear();
...
Box2d Polygon2d::MinAreaBoundingBox() const {
  ...
  if (!is_convex_) {
    static Polygon2d convex_polygon;
    convex_polygon.clear();
    (void)ComputeConvexHull(points_, &convex_polygon);
    mph_assert(convex_polygon.is_convex());
...
Polygon2d Polygon2d::ExpandByDistance(const double distance) const {
  ...
  static Polygon2d new_polygon;
  new_polygon.clear();
  bool success = ComputeConvexHull(points, &new_polygon);
  mph_assert(success);
  if (!success) {
    return Polygon2d();
  }
