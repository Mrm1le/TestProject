...
using msquare::planning_math::Vec2d;
...
bool Spline2dConstraint::Add2dBoundary(
  ...
  for (uint32_t i = 0; i < t_coord.size(); ++i) {
    // std::cout << "-------------------------" << std::endl;
    // std::cout << "ref_point.x: " << (ref_point[i]).x() << "  ref_point.y: "
    // << (ref_point[i]).y() << std::endl;
    const double d_lateral = SignDistance(ref_point[i], angle[i]);
    // std::cout << " d_lateral: " << d_lateral << std::endl;
    const double d_longitudinal =
        SignDistance(ref_point[i], angle[i] - M_PI / 2.0);
    // std::cout << " d_longitudinal : " << d_longitudinal << std::endl;
    ...
    // std::cout << "longi_coef: " << std::endl;
    // for (int i = 0; i < longi_coef.size(); i++) {
    //   std::cout << longi_coef[i] << std::endl;
    // }
    std::vector<double> longitudinal_coef =
        AffineCoef(angle[i] - M_PI / 2, rel_t);
    // std::cout << "longitudinal_coef : " << std::endl;
    // for (int i = 0; i < longitudinal_coef.size(); i++) {
    //   std::cout << longitudinal_coef[i] << std::endl;
    // }
    ...
    // std::cout << "[zyl debug]lateral_bound[i]: " << lateral_bound[i] << "
    // longitudinal_bound[i]: "
    //           << longitudinal_bound[i] << std::endl;
    ...
    if (i >= t_coord.size() - 3) {
      affine_boundary(4 * i, 0) = d_lateral - 0.00001;
      affine_boundary(4 * i + 1, 0) = -d_lateral - 0.00001;
      affine_boundary(4 * i + 2, 0) = d_longitudinal - 0.0001;
      affine_boundary(4 * i + 3, 0) = -d_longitudinal - 0.0001;
    }
    // affine_boundary(4 * i, 0) = d_lateral;
    // affine_boundary(4 * i + 1, 0) = -d_lateral - 2.0;
    // affine_boundary(4 * i + 2, 0) = d_longitudinal - longitudinal_bound[i];
    // affine_boundary(4 * i + 3, 0) = -d_longitudinal - longitudinal_bound[i];
  }
  // print affine_boundary:
  // for (uint32_t i = 0; i < 4 * t_coord.size(); i++) {
  //   std::cout << "affine_boundary:" << affine_boundary(i, 0) << std::endl;
  // }
...
bool Spline2dConstraint::AddSecondDerivativeSmoothConstraint(
    const std::vector<planning_math::Vec2d> &ref_point) {
  if (t_knots_.size() < 3) {
    return true;
  }
  Eigen::MatrixXd affine_equality =
      Eigen::MatrixXd::Zero(6 * (t_knots_.size() - 2) + 2, total_param_);
  Eigen::MatrixXd affine_boundary =
      Eigen::MatrixXd::Zero(6 * (t_knots_.size() - 2) + 2, 1);
  affine_boundary(6 * (t_knots_.size() - 2), 0) = ref_point.back().x(); // zyl
  affine_boundary(6 * (t_knots_.size() - 2) + 1, 0) =
      ref_point.back().y(); // zyl
  ...
  for (uint32_t i = 0; i + 2 < t_knots_.size(); ++i) {
    ...
    if (i == t_knots_.size() - 3) { // zyl
      double rel_t = t_knots_[i + 2] - t_knots_[i + 1];
      std::vector<double> power_t = PolyCoef(rel_t);
      for (uint32_t j = 0; j < num_params; ++j) {
        affine_equality(6 * i + 6, index_offset + 2 * num_params + j) =
            power_t[j];
        affine_equality(6 * i + 7, index_offset + 3 * num_params + j) =
            power_t[j];
      }
    }
  }
...
uint32_t Spline2dConstraint::FindIndex(const double t) const {
  ...
  return std::min(static_cast<uint32_t>((int)t_knots_.size() - 1),
...
std::vector<double> Spline2dConstraint::AffineCoef(const double angle,
  ...
  double x_coef = -planning_math::sin(planning_math::Angle16::from_rad(angle));
  double y_coef = planning_math::cos(planning_math::Angle16::from_rad(angle));
...
std::vector<double>
Spline2dConstraint::AffineDerivativeCoef(const double angle,
                                         const double t) const {
  const uint32_t num_params = spline_order_ + 1;
  std::vector<double> result(num_params * 2, 0.0);
  double x_coef = -planning_math::sin(planning_math::Angle16::from_rad(angle));
...
std::vector<double>
Spline2dConstraint::AffineSecondDerivativeCoef(const double angle,
                                               const double t) const {
  const uint32_t num_params = spline_order_ + 1;
  std::vector<double> result(num_params * 2, 0.0);
  double x_coef = -planning_math::sin(planning_math::Angle16::from_rad(angle));
...
std::vector<double>
Spline2dConstraint::AffineThirdDerivativeCoef(const double angle,
                                              const double t) const {
  const uint32_t num_params = spline_order_ + 1;
  std::vector<double> result(num_params * 2, 0.0);
  double x_coef = -planning_math::sin(planning_math::Angle16::from_rad(angle));
...
double Spline2dConstraint::SignDistance(const Vec2d &xy_point,
  ...
  return planning_math::InnerProd(
      xy_point.x(), xy_point.y(),
      -planning_math::sin(planning_math::Angle16::from_rad(angle)),
      planning_math::cos(planning_math::Angle16::from_rad(angle)));
}
...
std::vector<double>
Spline2dConstraint::SecondDerivativeCoef(const double t) const {
  std::vector<double> result(spline_order_ + 1, 0.0);
  std::vector<double> power_t = PolyCoef(t);
...
std::vector<double>
Spline2dConstraint::ThirdDerivativeCoef(const double t) const {
  std::vector<double> result(spline_order_ + 1, 0.0);
  std::vector<double> power_t = PolyCoef(t);
