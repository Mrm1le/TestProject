#include "common/refline/refline_smoother/spline_seg_kernel.h"

#include <vector>

namespace msquare {

void SplineSegKernel::init() {
  if (!initialized_) {
...
Eigen::MatrixXd
SplineSegKernel::SecondOrderDerivativeKernel(const uint32_t num_params,
                                             const double accumulated_x) {
  if (num_params > reserved_order_ + 1) {
...
Eigen::MatrixXd
SplineSegKernel::ThirdOrderDerivativeKernel(const uint32_t num_params,
                                            const double accumulated_x) {
  if (num_params > reserved_order_ + 1) {
