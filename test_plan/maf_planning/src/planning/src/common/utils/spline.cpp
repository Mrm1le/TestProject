...
double &band_matrix::operator()(int i, int j) {
  ...
  mph_assert((i >= 0) && (i < dim()) && (j >= 0) && (j < dim()));
  mph_assert((-num_lower() <= k) && (k <= num_upper()));
...
void band_matrix::lu_decompose() {
  ...
  for (int i = 0; i < this->dim(); i++) {
    mph_assert(this->operator()(i, i) != 0.0);
  ...
  for (int k = 0; k < this->dim(); k++) {
    ...
    for (int i = k + 1; i <= i_max; i++) {
      mph_assert(this->operator()(k, k) != 0.0);
...
std::vector<double> band_matrix::l_solve(const std::vector<double> &b) const {
  mph_assert(this->dim() == (int)b.size());
...
void spline::set_boundary(spline::bd_type left, double left_value,
  ...
  mph_assert(m_x.size() == 0); // set_points() must not have happened yet
  m_left = left;
  m_right = right;
...
void spline::set_points(const std::vector<double> &x,
  ...
  double x_min = x[0];
  double x_max = x.back();
  double dx = (x_max - x_min) / DISCRETIZATION;
  new_x.resize(DISCRETIZATION + 1);
  new_y.resize(DISCRETIZATION + 1);
  for (int i = 0; i <= DISCRETIZATION; i++) {
    double cur_x = x_min + dx * i;
    new_x[i] = cur_x;
    new_y[i] = calc_old(cur_x);
  }
  set_points_old(new_x, new_y, cubic_spline);
}
double spline::operator()(double x) const {
  size_t n = m_x.size();
  // find the closest point m_x[idx] < x, idx=0 even if x<m_x[0]
  int idx = std::max(int(std::floor((x - m_x[0]) / (m_x[1] - m_x[0]))), 0);
  double h = x - m_x[idx];
  double interpol;
  if (x < m_x[0]) {
...
double spline::calc_old(double x) const {
  size_t n = m_x.size();
  // find the closest point m_x[idx] < x, idx=0 even if x<m_x[0]
...
void spline::set_points_old(const std::vector<double> &x,
  ...
  if (cubic_spline == true) { // cubic spline interpolation
    ...
    for (int i = 1; i < n - 1; i++) {
      bm(i, i - 1) = 1.0 / 3.0 * (x[i] - x[i - 1]);
      bm(i, i) = 2.0 / 3.0 * (x[i + 1] - x[i - 1]);
      bm(i, i + 1) = 1.0 / 3.0 * (x[i + 1] - x[i]);
    ...
    if (m_right == spline::second_deriv) {
      ...
      bm(n - 1, n - 1) = 2.0 * (x[n - 1] - x[n - 2]);
      bm(n - 1, n - 2) = 1.0 * (x[n - 1] - x[n - 2]);
      rhs[n - 1] =
