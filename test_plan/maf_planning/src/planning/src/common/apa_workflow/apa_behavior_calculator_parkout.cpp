...
bool APABehaviorCalculatorParkOut::calculate_reached_takeover_point(
  ...
  double xy_diff_from_init = std::hypot(ego_pose.x - g_park_out_init.x,
                                        ego_pose.y - g_park_out_init.y);
  ...
  const double width = CarParams::GetInstance()->vehicle_width_real +
                       parkout_takeover_point_lat_inflation;

  //"back_edge_to_center" is actually rear_bumper_to_rear_axle
  //
  //   in src/planning/src/common/config/vehicle_param.cpp Function
  //   VehicleParam::loadFile
  ...
  planning_math::Box2d check_area =
      planning_math::Box2d(ego_center_axis, width);
...
bool APABehaviorCalculatorParkOut::check_parkout_direction(uint32_t direction) {
  ...
  for (const auto &obstacle : obstacles) {
    if (msquare::CarParams::GetInstance()
            ->car_config.apoa_config.is_direct_dynamic_person_ignore) {
    ...
    if (check_area.HasOverlap(obstacle->PerceptionBoundingBox())) {
      MSD_LOG(ERROR,
              "[check_parkout_direction] collision with PerceptionBoundingBox");
      return false;
    }
  }
  ...
  for (auto &obs : world_model_->get_parking_ground_line_fusion()) {
    ...
    for (unsigned int i = 0; i < obs.pts.size(); i = i + 2) {
      ...
      if (check_area.HasOverlap(uss_ground_line)) {
        MSD_LOG(ERROR,
                "[check_parkout_direction] collision with uss_ground_line");
        return false;
      }
    }
  }
...
bool APABehaviorCalculatorParkOut::calc_parkout_target_perpendicular_oblique(
  ...
  if (parkout_type == ParkOutType::PERPENDICULAR_OBLIQUE_FRONT ||
    ...
    const double straight_offset =
        oblique ? PERPENDICULAR_OFFSET : OBLIQUE_OFFSET;
    ...
    planning_math::Vec2d target_xy =
        ego_position + dir * ego_vec * straight_offset;
  ...
  planning_math::Vec2d corner0{parking_slot_info.original_corners[0].x,
                               parking_slot_info.original_corners[0].y};
  planning_math::Vec2d corner1{parking_slot_info.original_corners[1].x,
                               parking_slot_info.original_corners[1].y};
  planning_math::Vec2d corner2{parking_slot_info.original_corners[2].x,
                               parking_slot_info.original_corners[2].y};
  planning_math::Vec2d corner3{parking_slot_info.original_corners[3].x,
                               parking_slot_info.original_corners[3].y};
  // virtual corner replacing real corner in rectangle oblique
  if (oblique && parking_slot_info.virtual_corner.slot_type == 4) {
    planning_math::Vec2d virtual_corner{parking_slot_info.virtual_corner.p.x,
                                        parking_slot_info.virtual_corner.p.y};
  ...
  const planning_math::Vec2d corner0_in_ego =
      planning_math::tf2d(ego_pose, corner0);
  const planning_math::Vec2d corner1_in_ego =
      planning_math::tf2d(ego_pose, corner1);
  const planning_math::Vec2d corner2_in_ego =
      planning_math::tf2d(ego_pose, corner2);
  const planning_math::Vec2d corner3_in_ego =
      planning_math::tf2d(ego_pose, corner3);
  ...
  //
  const bool edge_03_is_in_ego_front = corner0_in_ego.x() + corner3_in_ego.x() >
                                       corner1_in_ego.x() + corner2_in_ego.x();
  ...
  //   in src/planning/src/common/config/vehicle_param.cpp Function
  //   VehicleParam::loadFile
  const double rear_bumper_to_rear_axle =
      VehicleParam::Instance()->back_edge_to_center;
  const double vehicle_width = VehicleParam::Instance()->width;
  const double front_bumper_to_rear_axle =
      VehicleParam::Instance()->length - rear_bumper_to_rear_axle;

  double bumper_to_axle_offset =
      parkout_is_front ? rear_bumper_to_rear_axle : front_bumper_to_rear_axle;
...
bool APABehaviorCalculatorParkOut::calc_parkout_target_parallel(
  ...
  const planning_math::Vec2d corner0{parking_slot_info.original_corners[0].x,
                                     parking_slot_info.original_corners[0].y};
  const planning_math::Vec2d corner1{parking_slot_info.original_corners[1].x,
                                     parking_slot_info.original_corners[1].y};
  const planning_math::Vec2d corner2{parking_slot_info.original_corners[2].x,
                                     parking_slot_info.original_corners[2].y};
  const planning_math::Vec2d corner3{parking_slot_info.original_corners[3].x,
                                     parking_slot_info.original_corners[3].y};
  ...
  //
  bool edge_03_is_in_ego_left = corner0_in_ego.y() + corner3_in_ego.y() >
                                corner1_in_ego.y() + corner2_in_ego.y();
  bool parkout_is_parallel_left =
  ...
  const double vehicle_width = VehicleParam::Instance()->width;
  const double offset_lon =
      0.5 * (PARALLEL_OFFSET_LON_MIN + PARALLEL_OFFSET_LON_MAX) +
      rear_bumper_to_rear_axle;
  const double offset_lat =
      0.5 * (PARALLEL_OFFSET_LAT_MIN + PARALLEL_OFFSET_LAT_MAX) +
      0.5 * vehicle_width;
  ...
  double error_tolerence_x =
      0.5 * (PARALLEL_OFFSET_LON_MAX - PARALLEL_OFFSET_LON_MIN);
  double error_tolerence_y =
      0.5 * (PARALLEL_OFFSET_LAT_MAX - PARALLEL_OFFSET_LAT_MIN);
  double error_tolerence_yaw = 5.0 * M_PI / 180.0;
...
bool APABehaviorCalculatorParkOut::calculate_parkout_target(
  ...
  if (slot_type == ParkingSlotType::PERPENDICULAR) {
    ...
    success = calc_parkout_target_perpendicular_oblique(target_pose,
                                                        error_tolerence, false);
...
uint32_t APABehaviorCalculatorParkOut::calculate_recommended_parkout_direction(
  ...
  planning_math::Vec2d left_vector =
      (corner_pts[0] + corner_pts[1] - corner_pts[2] - corner_pts[3]) / 2;
  planning_math::Vec2d up_vector =
      (corner_pts[0] + corner_pts[3] - corner_pts[2] - corner_pts[1]) / 2;
  ...
  MSD_LOG(INFO,
          "[CheckRecommendParkoutDirection] up_unit x:%f y:%f heading_vector "
          "x:%f y:%f",
          up_unit.x(), up_unit.y(), heading_vector.x(), heading_vector.y());
  MSD_LOG(INFO, "[CheckRecommendParkoutDirection] left_vector x:%f y:%f",
          left_vector.x(), left_vector.y());
  MSD_LOG(INFO,
          "[CheckRecommendParkoutDirection] last heading_vector x:%f y:%f",
          last_heading_vector.x(), last_heading_vector.y());
  ...
  if (g_slot_type == ParkingSlotType::PARALLEL) {
    ...
    if (up_unit.InnerProd(last_heading_vector) < 0.0) {
      prioritized_directions.push_back(
          ParkOutType::PERPENDICULAR_OBLIQUE_REAR_RIGHT);
      prioritized_directions.push_back(
          ParkOutType::PERPENDICULAR_OBLIQUE_REAR_LEFT);
      prioritized_directions.push_back(ParkOutType::PERPENDICULAR_OBLIQUE_REAR);
      prioritized_directions.push_back(
          ParkOutType::PERPENDICULAR_OBLIQUE_FRONT_RIGHT);
      prioritized_directions.push_back(
          ParkOutType::PERPENDICULAR_OBLIQUE_FRONT_LEFT);
      prioritized_directions.push_back(
          ParkOutType::PERPENDICULAR_OBLIQUE_FRONT);
    } else {
      prioritized_directions.push_back(
          ParkOutType::PERPENDICULAR_OBLIQUE_REAR_RIGHT);
      prioritized_directions.push_back(
...
uint32_t APABehaviorCalculatorParkOut::calculate_available_parkout_directions(
...
ParkoutSceneType APABehaviorCalculatorParkOut::calc_parkout_scene_type(
  ...
  double ltheta =
      atan2(corners[0].y - corners[3].y, corners[0].x - corners[3].x);
  ...
  planning_math::Vec2d ego_position(ego_pose.x, ego_pose.y);
  planning_math::Vec2d ego_heading_vector =
      planning_math::Vec2d::CreateUnitVec2d(ego_pose.theta);
  planning_math::Vec2d heading_vector =
      planning_math::Vec2d::CreateUnitVec2d(ltheta);
  ...
  planning_math::Box2d check_box_side =
      planning_math::Box2d(center_axis, side_check_region);
  planning_math::Box2d check_box_side_reverse =
      planning_math::Box2d(center_axis, side_check_region);
    ...
    if (obstacle->IsStatic() == 1 &&
      ...
      if (!is_collision_side) {
        if (check_box_side.HasOverlap(obstacle->PerceptionBoundingBox())) {
          ...
          MSD_LOG(ERROR, "[%s] side collide with PerceptionBoundingBox",
                  __FUNCTION__);
        }
      }
      if (!is_collision_side_reverse) {
        if (check_box_side_reverse.HasOverlap(
                obstacle->PerceptionBoundingBox())) {
          is_collision_side_reverse = true;
          MSD_LOG(ERROR, "[%s] side reverse collide with PerceptionBoundingBox",
                  __FUNCTION__);
        }
      }
    }
  ...
  if (is_collision_side && is_collision_side_reverse) {
    MSD_LOG(ERROR, "[%s] side both collide with PerceptionBoundingBox",
            __FUNCTION__);
    ...
    for (int i = 0; i < obs.pts.size(); i++) {
      ...
      if (!is_collision_side_reverse) {
        if (check_box_side_reverse.IsPointIn(uss_point)) {
          MSD_LOG(ERROR, "[%s] side reverse collide with uss points",
                  __FUNCTION__);
          is_collision_side_reverse = true;
        }
      }
    ...
    for (unsigned int i = 0; i < obs.pts.size(); i = i + 2) {
      ...
      if (!is_collision_side_reverse) {
        if (check_box_side_reverse.HasOverlap(uss_ground_line)) {
          MSD_LOG(ERROR, "[%s] side reverse collide with obstacle line",
                  __FUNCTION__);
          is_collision_side_reverse = true;
        }
      }
  ...
  MSD_LOG(ERROR, "[%s] is_collision_side_reverse: %d", __FUNCTION__,
          is_collision_side_reverse);

  if (is_collision_side && is_collision_side_reverse) {
    MSD_LOG(ERROR, "[%s] side both collide with ground_line_fusion",
            __FUNCTION__);
    is_collision_side_both = true;
  }
