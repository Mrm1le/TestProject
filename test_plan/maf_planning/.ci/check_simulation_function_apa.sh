...
#declare -r onboard_branch="Lacar/Deliver_V4.21"
declare -r onboard_branch="cpp/main-dev-apa"
declare -r ctrl_branch="main_0721_daily_build"
...
if [ "${ci_type}" = "PullRequest" ];then 
    ...
    docker run -i --rm -v ${offboard_dir}:/opt artifactory.momenta.works/docker-msd-dev/python:npp_sim bash -c "ls /opt && cd /opt/legacy_code/mddp/script/zhouzihan && python3 submit_sim_task.py --main_branch ${onboard_branch} --main_commit $onboard_commit --usr_name '${user_name}'  --start_time $start_time  --trigger_type ${trigger_type} --datasets 'apa_small,apa_full' --ctrl_branch ${ctrl_branch}"
fi