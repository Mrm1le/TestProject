...
vector<Circle> rnsac(vector<mypoint2f> points, int Nmin, int imax, float tau, int d, int num)
{
    ...
    for (int m = 0; m < num; m++)
    {
        ...
        cout << endl
             << "Elected Circle " << qualifiedCircle << " Elected with " << voteNum << " votes" << endl
             << endl;
...
std::vector<mypoint2f> read_trajectory(const std::string &filename)
{
    std::vector<mypoint2f> points;
    std::ifstream file(filename);
    double x, y, angle;
    while (file >> x >> y >> angle)
    {
        points.emplace_back(mypoint2f{x, y, angle});
    }
    return points;
}
...
int main(int argc, const char *argv[])
{
    ...
    // }
    auto points = read_trajectory("/home/thj/Desktop/TestProject/test_curvefitting/RANSAC-Algorithm-master/trajectory.txt");
    std::cout << points.size() << std::endl;
    vector<Circle> Circles = rnsac(points, 3, 1000, 0.05, 0, 15);
